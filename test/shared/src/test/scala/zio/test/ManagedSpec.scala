...
object ManagedSpec extends DefaultRuntime {
  ...
  def provideManaged[R, E, L, S](
    ...
    def loop(r: R, spec: Spec[R, E, L, ZIO[R, E, S]]): ZIO[Any, E, Spec[Any, E, L, ZIO[Any, E, S]]] =
      spec.caseValue match {
        case SuiteCase(label, specs, exec) =>
          ZIO.foreach(specs)(loop(r, _)).map(z => Spec.suite(label, z.toVector, exec))
    ...
    spec.caseValue match {
      case SuiteCase(label, specs, exec) =>
        Spec.effect(r.use(r => ZIO.foreach(specs)(loop(r, _))).map(z => SuiteCase(label, z.toVector, exec)))
