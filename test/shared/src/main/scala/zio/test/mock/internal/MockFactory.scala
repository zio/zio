...
import zio.{ Has, IO, Promise, UIO, ZIO }

/**
 * Extracted concern of handling mock state. Given initial `State[R]`,
 * a `MockFactory` constructs a mock running that state.
 */
object MockFactory {
  ...
  def make[R <: Has[_]](state: State[R]): Mock =
    new Mock {
