...
final case class Sample[-R, +A](value: A, shrink: ZStream[R, Nothing, Sample[R, A]]) { self =>
  ...
  final def collect[B](pf: PartialFunction[A, B]): ZStream[R, Nothing, Sample[R, B]] =
    pf.andThen(b => ZStream(Sample(b, shrink.flatMap(_.collect(pf)))))
      .applyOrElse(value, (_: A) => shrink.flatMap(_.collect(pf)))
