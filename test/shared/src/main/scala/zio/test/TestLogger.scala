...
import zio.{Console, Has, UIO, URIO, ZIO, ZServiceBuilder, ZTraceElement}
...
object TestLogger {
  ...
  def fromConsole(implicit trace: ZTraceElement): ZServiceBuilder[Has[Console], Nothing, Has[TestLogger]] =
    ZIO
      .service[Console]
      .map { console =>
        new TestLogger {
