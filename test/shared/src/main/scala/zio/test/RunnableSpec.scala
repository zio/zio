...
abstract class RunnableSpec[R, E] extends AbstractRunnableSpec {
  ...
  final def main(args: Array[String]): Unit = {
    ...
    if (TestPlatform.isJVM) {
      val exitCode = runtime.unsafeRun(run(spec, testArgs).provideService(runner.bootstrap))
      doExit(exitCode)
    } else if (TestPlatform.isJS) {
      runtime.unsafeRunAsyncWith[Nothing, Int](run(spec, testArgs).provideService(runner.bootstrap)) { exit =>
