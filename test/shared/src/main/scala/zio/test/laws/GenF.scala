...
object GenF {
  ...
  val option: GenF[Random, Option] =
    new GenF[Random, Option] {
      def apply[R1 <: Random, A](gen: Gen[R1, A])(implicit trace: ZTraceElement): Gen[R1, Option[A]] =
        Gen.option(gen)
    }
  ...
  val set: GenF[Random with Sized, Set] =
    new GenF[Random with Sized, Set] {
      def apply[R1 <: Random with Sized, A](gen: Gen[R1, A])(implicit trace: ZTraceElement): Gen[R1, Set[A]] =
        Gen.setOf(gen)
    }
