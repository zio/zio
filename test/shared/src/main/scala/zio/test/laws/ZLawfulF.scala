...
/**
 * `ZLawfulF[CapsF, Caps, R]` describes a set of laws that a parameterized type `F[A]` with capabilities `CapsF` is
 * expected to satisfy with respect to all types `A` that have capabilities `Caps`. Lawful instances can be combined
