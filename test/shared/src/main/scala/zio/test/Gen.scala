...
import zio.{Chunk, NonEmptyChunk, Random, Trace, UIO, URIO, Unsafe, ZIO, Zippable}
...
final case class Gen[-R, +A](sample: ZStream[R, Nothing, Sample[R, A]]) { self =>
   ...
   * In nondeterministic mode, randomly selects one of the two generators.
   */
...
object Gen extends GenZIO with FunctionVariants with TimeVariants {
  ...
  private val deterministic: FiberRef[Boolean] =
    FiberRef.unsafe.make(true)(Unsafe.unsafe)

  /**
   * Constructs a dual generator that behaves differently depending on whether
   * deterministic or nondeterministic sampling is in effect. In deterministic
   * mode (e.g. `checkAll`, `runCollect`), the first generator is used. In
   * nondeterministic mode (e.g. `check`, `runCollectN`), the second generator
   * is used.
   */
  def dual[R, A](
    ...
    Gen(ZStream.unwrap {
      Gen.deterministic.get.map(if (_) deterministicGen.sample else nondeterministicGen.sample)
    })
   ...
   * specified generators. In nondeterministic mode, randomly selects from
   * among the provided generators.
   */
  ...
  /**
   * Constructs a deterministic generator from the specified iterable of fixed
   * values. In deterministic mode, all values are enumerated. In
   * nondeterministic mode, a random element is selected.
   */
  def fromIterable[R, A](
    as: Iterable[A],
    shrinker: A => ZStream[R, Nothing, A] = defaultShrinker
  )(implicit trace: Trace): Gen[R, A] = {
    val deterministicGen =
    ...
    Gen.dual(
      ...
      if (as.knownSize >= 0)
        ...
        Gen.fromZIOSample {
          ZIO.randomWith(_.nextIntBounded(100)).map { n =>
            val it = as.iterator.drop(n)
            Sample.noShrink(if (it.hasNext) it.next() else as.iterator.next())
          }
        }
    )
  }
