...
object MessageDesc {
  ...
  def result[A](render: A => String): MessageDesc[A] = Result {
    ...
    case Right(value) => render(value)
  }
  def either[A](render: Either[Throwable, A] => String): MessageDesc[A] = Result(render)
  ...
  private final case class Wrap[-A](render: Either[Throwable, A] => MessageDesc[A]) extends MessageDesc[A]
  private final case class Result[-A](render: Either[Throwable, A] => String)       extends MessageDesc[A]
...
sealed trait MessageDesc[-A] { self =>
  ...
  def render(a: A, isSuccess: Boolean): Message                         = renderLine(Right(a), isSuccess).toMessage
  def render(either: Either[Throwable, A], isSuccess: Boolean): Message = renderLine(either, isSuccess).toMessage

  private def renderLine(a: Either[Throwable, A], isSuccess: Boolean): Line =
    self match {
      case MessageDesc.Wrap(render) => render(a).renderLine(a, isSuccess)
