...
abstract class ZIOSpecAbstract extends ZIOApp {
  ...
  final def <>(that: ZIOSpecAbstract)(implicit trace: ZTraceElement): ZIOSpecAbstract =
    new ZIOSpecAbstract {
      ...
      override def runSpec: ZIO[
        Environment with TestEnvironment with ZIOAppArgs with StreamingTestOutput with TestLogger with Scope,
        Any,
        Any
      ] =
  ...
  protected def runSpec: ZIO[
    Environment with TestEnvironment with ZIOAppArgs with StreamingTestOutput with TestLogger with ExecutionEventSink with Scope,
    Any,
    Any
  ] = {
    implicit val trace = Tracer.newTrace
  ...
  private[zio] def runSpec(
    spec: ZSpec[
      Environment with TestEnvironment with ZIOAppArgs with StreamingTestOutput with TestLogger with Clock with Scope with ExecutionEventSink,
      Any
    ],
    testArgs: TestArgs
  )(implicit
    ...
    Environment with TestEnvironment with ZIOAppArgs with StreamingTestOutput with TestLogger with Scope with ExecutionEventSink,
    Summary
  ] = {
