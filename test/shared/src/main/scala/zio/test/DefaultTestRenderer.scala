...
object DefaultTestRenderer {
  def apply[E](): TestRenderer[E] = new TestRenderer[E] {
    override def render(
        ...
        executedSpec.caseValue match {
          case c @ Spec.SuiteCase(label, executedSpecs, _) =>
            for {
              ...
              renderedAnnotations = annotationRenderer.run(ancestors, annotations)
          ...
          case Spec.TestCase(label, result, annotations) =>
            result.flatMap { result =>
              val renderedAnnotations = annotationRenderer.run(ancestors, annotations)
              val renderedResult = result match {
                case Right(TestSuccess.Succeeded(_)) =>
                  UIO.succeedNow(rendered(Test, label, Passed, depth, renderSuccessLabel(label, depth)))
    ...
    override def renderStats(duration: Duration, executedSpec: ExecutedSpec[E]): UIO[String] = {
      ...
      } yield renderStatsTotal(duration, success, ignore, failure, total)
    }
  }

  private def renderStatsTotal[E](duration: Duration, success: Int, ignore: Int, failure: Int, total: Int) =
    cyan(
