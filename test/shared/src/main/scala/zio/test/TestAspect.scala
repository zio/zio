...
/**
 * A `TestAspect` is an aspect that can be weaved into specs. You can think of an aspect as a polymorphic function,
 * capable of transforming one test into another, possibly enlarging the environment or error type.
 */
...
object TestAspect extends TimeoutVariants {
  ...
  /**
   * Constructs an aspect that evaluates every test between two effects, `before` and `after`, where the result of
   * `before` can be used in `after`.
   */
  def around[R0, E0, A0](
    before: ZIO[R0, E0, A0]
  ...
  /**
   * Constructs an aspect that evaluates all tests between two effects, `before` and `after`, where the result of
   * `before` can be used in `after`.
   */
  ...
  /**
   * An aspect that runs each test on a separate fiber and prints a fiber dump if the test fails or has not terminated
  ...
  /**
   * An aspect that runs each test with the `TestConsole` instance in the environment set to debug mode so that console
   * output is rendered to standard output in addition to being written to the output buffer.
   */
  ...
  /**
   * An aspect that sets suites to the specified execution strategy, but only if their current strategy is inherited
   * (undefined).
   */
  ...
  /**
   * An aspect that makes a test that failed for any reason pass. Note that if the test passes this aspect will make it
   * fail.
   */
  ...
  /**
   * An aspect that makes a test that failed for the specified failure pass. Note that the test will fail for other
   * failures and also if it passes correctly.
   */
  ...
  /**
   * An aspect that records the state of fibers spawned by the current test in [[TestAnnotation.fibers]]. Applied by
   * default in [[DefaultRunnableSpec]] and [[MutableRunnableSpec]] but not in [[RunnableSpec]]. This aspect is required
   * for the proper functioning of `TestClock.adjust`.
   */
  ...
  /**
   * An aspect that runs each test with the maximum number of shrinkings to minimize large failures set to the specified
   * value.
   */
  ...
  /**
   * An aspect that repeats the test a default number of times, ensuring it is stable ("non-flaky"). Stops at the first
   * failure.
   */
  ...
  /**
   * An aspect that repeats the test a specified number of times, ensuring it is stable ("non-flaky"). Stops at the
   * first failure.
   */
  ...
  /**
   * An aspect that runs each test with the number of times to repeat tests to ensure they are stable set to the
   * specified value.
   */
  ...
  /**
   * An aspect that restores a given [[zio.test.environment.Restorable Restorable]] 's state to its starting state after
   * the test is run. Note that this is only useful when repeating tests.
   */
  def restore[R0](service: R0 => Restorable): TestAspectAtLeastR[R0] =
    around(ZIO.accessM[R0](r => service(r).save))(restore => restore)

  /**
   * An aspect that restores the [[zio.test.environment.TestClock TestClock]] 's state to its starting state after the
  ...
  /**
   * An aspect that restores the [[zio.test.environment.TestRandom TestRandom]] 's state to its starting state after the
   * test is run. Note that this is only useful when repeating tests.
   */
  ...
  /**
   * An aspect that restores the [[zio.test.environment.TestSystem TestSystem]] 's state to its starting state after the
   * test is run. Note that this is only useful when repeating tests.
   */
  ...
  /**
   * An aspect that restores all state in the standard provided test environments ([[zio.test.environment.TestClock
   * TestClock]], [[zio.test.environment.TestConsole TestConsole]], [[zio.test.environment.TestRandom TestRandom]] and
   * [[zio.test.environment.TestSystem TestSystem]]) to their starting state after the test is run. Note that this is
   * only useful when repeating tests.
   */
  ...
  /**
   * An aspect that runs each test with the number of sufficient samples to check for a random variable set to the
   * specified value.
   */
  ...
  /**
   * An aspect that runs each test with the `TestConsole` instance in the environment set to silent mode so that console
   * output is only written to the output buffer and not rendered to standard output.
   */
  ...
  object PerTest {
    ...
    /**
     * A `PerTest.Poly` is a `TestAspect.PerTest` that is completely polymorphic, having no requirements ZRTestEnv on
