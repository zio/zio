...
trait LivePlatformSpecific {
  ...
  def default: ZLayer[ZEnv, Nothing, Live] =
    ZLayer.fromFunction { (clock: Clock.Service, console: Console.Service, random: Random.Service, system: System.Service, blocking: Blocking.Service) =>
      Has(new Live.Service {
        val env = Has.allOf(clock, console, random, system, blocking)
