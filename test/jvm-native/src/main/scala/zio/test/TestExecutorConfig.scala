...
object TestExecutorConfig {
  ...
  /**
   * Reads the `zio.test.executor` system property and returns the corresponding
   * executor type.
   *
   * @return
   *   Some(ExecutorType) if property is set, None otherwise
   * @throws IllegalArgumentException
   *   if the property value is not recognized
   */
