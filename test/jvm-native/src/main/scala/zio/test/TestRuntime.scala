...
/**
 * Test-specific runtime configuration that allows overriding the default executor.
 */
object TestRuntime {
  ...
  /**
   * The default executor for tests. This delegates to the configured test executor.
   * Test files should use `TestRuntime.defaultExecutor` instead of `Runtime.defaultExecutor`.
   */
  ...
  /**
   * Sets the default executor to use for tests.
   * This should be called during test bootstrap before any tests run.
   */
