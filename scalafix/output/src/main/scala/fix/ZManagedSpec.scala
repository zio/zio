  ...
  def spec: ZSpec[Environment, Failure] = suite("ZManaged")(
    ...
    suite("ensuringFirst")(
      ...
      test("Runs on failures") {
        for {
          effects <- Ref.make[List[String]](Nil)
          _ <-
            ZManaged.fromZIO(ZIO.fail(())).ensuringFirst(effects.update("Ensured" :: _)).useDiscard(ZIO.unit).either
          result <- effects.get
        } yield assert(result)(equalTo(List("Ensured")))
      },
      test("Works when finalizers have defects") {
    ...
    suite("retry")(
      test("Should retry the reservation") {
        for {
          ...
          program =
            ZManaged
              .acquireReleaseWith(retries.updateAndGet(_ + 1).flatMap(r => ZIO.fail(()).unless(r == 3)))(_ => ZIO.unit)
    ...
    suite("toLayerMany")(
      test("converts a managed effect to a layer") {
        ...
        val layer   = managed.toProviderMany
    ...
    suite("collect")(
      test("collectM maps value, if PF matched") {
      ...
      test("collectM produces given error, if PF not matched") {
