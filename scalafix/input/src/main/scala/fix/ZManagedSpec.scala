...
object ZManagedSpec extends DefaultRunnableSpec {
  ...
  def spec: ZSpec[Environment, Failure] = suite("ZManaged")(
    ...
    suite("ensuringFirst")(
      ...
      testM("Runs on failures") {
        for {
          effects <- Ref.make[List[String]](Nil)
          _ <-
            ZManaged.fromEffect(ZIO.fail(())).ensuringFirst(effects.update("Ensured" :: _)).use_(ZIO.unit).either
          result <- effects.get
        } yield assert(result)(equalTo(List("Ensured")))
      },
      testM("Works when finalizers have defects") {
    ...
    suite("retry")(
      testM("Should retry the reservation") {
        for {
          ...
          program =
            ZManaged
              .make(retries.updateAndGet(_ + 1).flatMap(r => ZIO.fail(()).unless(r == 3)))(_ => ZIO.unit)
    ...
    suite("toDepsMany")(
      testM("converts a managed effect to a set of dependencies") {
    ...
    suite("withEarlyRelease")(
      ...
      testM("The canceler should run uninterruptibly") {
        for {
          ...
          latch <- Promise.make[Nothing, Unit]
          managed =
            Managed.make(ZIO.unit)(_ => latch.succeed(()) *> ZIO.never.whenM(ref.get)).withEarlyRelease
          result <- managed.use { case (canceler, _) =>
                      for {
