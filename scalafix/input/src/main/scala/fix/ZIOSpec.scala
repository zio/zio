  ...
  def spec: ZSpec[Environment, Failure] = suite("ZIOSpec")(
    ...
    suite("bracket")(
      testM("bracket happy path") {
        for {
          ...
          result   <-
            ZIO.bracket(IO.succeed(42), (_: Int) => release.set(true), (a: Int) => ZIO.effectTotal(a + 1))
    ...
    suite("provideSomeDeps")(
      testM("can split environment into two parts") {
        val clockDeps: ZDeps[Any, Nothing, Clock]    = Clock.live
        val zio: ZIO[Clock with Random, Nothing, Unit] = ZIO.unit
        val zio2: URIO[Random, Unit]                   = zio.provideSomeDeps[Random](clockDeps)
        assertM(zio2)(anything)
      }
    ),
    suite("raceAll")(
      ...
      testM("bracket release called on interrupt") {
        val io =
          for {
            ...
            p2 <- Promise.make[Nothing, Unit]
            fiber <-
              IO.bracket(IO.unit)(_ => p2.succeed(()) *> IO.unit)(_ => p1.succeed(()) *> IO.never).fork
            _ <- p1.await
            _ <- fiber.interrupt
            _ <- p2.await
          } yield ()
