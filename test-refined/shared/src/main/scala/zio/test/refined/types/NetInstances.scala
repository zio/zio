...
trait NetInstances {
  ...
  val portNumberGen: Gen[Has[Random], PortNumber]                   = Gen.int(0, 656535).map(Refined.unsafeApply)
  val systemPortNumberGen: Gen[Has[Random], SystemPortNumber]       = Gen.int(0, 1023).map(Refined.unsafeApply)
  val userPortNumberGen: Gen[Has[Random], UserPortNumber]           = Gen.int(1024, 49151).map(Refined.unsafeApply)
  ...
  val rfc5737Testnet1Gen: Gen[Has[Random], Rfc5737Testnet1] = _0to255.map(v => Refined.unsafeApply(s"192.0.2.$v"))
  val rfc5737Testnet2Gen: Gen[Has[Random], Rfc5737Testnet2] = _0to255.map(v => Refined.unsafeApply(s"198.51.100.$v"))
  val rfc5737Testnet3Gen: Gen[Has[Random], Rfc5737Testnet3] = _0to255.map(v => Refined.unsafeApply(s"203.0.113.$v"))
  val rfc5737TestnetGen: Gen[Has[Random], Rfc5737Testnet] = Gen
