 ...
 * application in a separate process and controlling/monitoring it.
 */
object ProcessTestUtils {
   ...
   *
   * @param process
   *   The underlying JVM process
   * @param outputCapture
   *   The captured stdout/stderr output
   * @param outputFile
   *   The file where output is being written
   */
  final case class AppProcess(
    ...
    /**
     * Helper to manually map returned exit codes to expected values This works
     * around platform inconsistencies in signal handling
     */
     ...
     *
     * @param signal
     *   The signal to send (e.g. "TERM", "INT", etc.)
     */
        ...
        for {
          ...
          _ <- if (pidOpt.isEmpty) {
                 ...
                 if (isWindows) {
                   ...
                   ZIO.attempt {
                     val signalFile =
                       new File(java.lang.System.getProperty("java.io.tmpdir"), s"zio-signal-${process.pid()}")
                       ...
                       ZIO.attempt {
                         ...
                         if (result != 0) {
                           throw new RuntimeException(
                             s"Failed to send SIGINT to process ${pid.pid()}, exit code: $result"
                           )
                         }
                       }
                     case "TERM" =>
                       ZIO.attempt {
                         ...
                         if (exitCode != 0) {
                           throw new RuntimeException(
                             s"Failed to send SIGTERM to process ${pid.pid()}, exit code: $exitCode"
                           )
                         }
                       }
                     case "KILL" =>
                       ZIO.attempt {
                         ...
                         if (exitCode != 0) {
                           throw new RuntimeException(
                             s"Failed to send SIGKILL to process ${pid.pid()}, exit code: $exitCode"
                           )
                         }
                       }
                     case other =>
                       ZIO.attempt {
                         ...
                         if (exitCode != 0) {
                           throw new RuntimeException(
                             s"Failed to send signal $other to process ${pid.pid()}, exit code: $exitCode"
                           )
                         }
                       }
                   }
    ...
    /**
     * Waits for the process to exit, with special handling to ensure all output
     * is captured and exit codes are normalized.
     *
     * @param timeout
     *   Maximum time to wait
     */
    def waitForExit(timeout: Duration = 30.seconds): Task[Int] =
      for {
        ...
        mappedExitCode <- if (output.contains("ZIO-SIGNAL:")) {
                            ...
                            val signalType =
                              if (output.contains("ZIO-SIGNAL: INT")) "INT"
   ...
   *
   * @param mainClass
   *   The fully qualified name of the ZIOApp class
   * @param gracefulShutdownTimeout
   *   Custom graceful shutdown timeout (if testing it)
   * @param jvmArgs
  ...
  /**
   * Creates a test application with configurable exit code behavior. This can
   * be used for testing exit codes explicitly.
   *
   * @param packageName
   *   Optional package name for the test application
   */
  ...
  /**
   * Creates a simple test application with configurable behavior. This can be
   * used to compile and run test applications dynamically.
   *
   * @param className
   *   The name of the class to generate
   * @param behavior
   *   The effect to run in the application
   * @param packageName
