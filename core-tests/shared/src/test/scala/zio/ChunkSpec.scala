  ...
  def spec = suite("ChunkSpec")(
    ...
    suite("dropWhileZIO")(
      test("dropWhileZIO happy path") {
        assertM(Chunk(1, 2, 3, 4, 5).dropWhileZIO(el => ZIO.succeed(el % 2 == 1)))(equalTo(Chunk(2, 3, 4, 5)))
      },
    ...
    suite("takeWhileZIO")(
      test("takeWhileZIO happy path") {
        assertM(Chunk(1, 2, 3, 4, 5).takeWhileZIO(el => ZIO.succeed(el % 2 == 1)))(equalTo(Chunk(1)))
      },
    ...
    suite("collectZIO")(
      test("collectZIO empty Chunk") {
        assertM(Chunk.empty[Nothing].collectZIO { case _ => ZIO.succeed(1) })(equalTo(Chunk.empty))
      },
    ...
    suite("collectWhileZIO")(
      test("collectWhileZIO empty Chunk") {
        assertM(Chunk.empty[Nothing].collectWhileZIO { case _ => ZIO.succeed(1) })(equalTo(Chunk.empty))
      },
