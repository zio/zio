  ...
  def spec: ZSpec[Environment, Failure] = suite("ChunkSpec")(
    suite("size/length")(
      test("concatenated size must match length") {
        ...
        assert(chunk.size == chunk.length)
      },
    ...
    testM("apply") {
      check(chunkWithIndex(Gen.unit)) { case (chunk, i) =>
        assert(chunk.apply(i) == chunk.toList.apply(i))
      }
    },
    ...
    test("splitWhere") {
      assert(Chunk(1, 2, 3, 4).splitWhere(_ == 2) == (Chunk(1) -> Chunk(2, 3, 4)))
    },
    testM("length") {
      check(largeChunks(intGen))(chunk => assert(chunk.length == chunk.toList.length))
    },
    testM("equality") {
    ...
    testM("materialize") {
      check(mediumChunks(intGen))(c => assert(c.materialize.toList == c.toList))
    },
    testM("foldLeft") {
      check(Gen.alphaNumericString, Gen.function2(Gen.alphaNumericString), smallChunks(Gen.alphaNumericString)) {
        (s0, f, c) => assert(c.fold(s0)(f) == c.toArray.foldLeft(s0)(f))
      }
    },
    test("foldRight") {
    ...
    test("mapAccum") {
      assert(Chunk(1, 1, 1).mapAccum(0)((s, el) => (s + el, s + el)) == (3 -> Chunk(1, 2, 3)))
    },
    suite("mapAccumM")(
    ...
    testM("map") {
      ...
      check(smallChunks(intGen), fn)((c, f) => assert(c.map(f).toList == c.toList.map(f)))
    },
    suite("mapM")(
      testM("mapM happy path")(checkM(mediumChunks(intGen), Gen.function(Gen.boolean)) { (chunk, f) =>
        chunk.mapM(s => UIO.succeed(f(s))).map(a => assert(a == chunk.map(f)))
      }),
      testM("mapM error") {
        Chunk(1, 2, 3).mapM(_ => IO.fail("Ouch")).either.map(a => assert(a == Left("Ouch")))
      } @@ zioTag(errors)
    ),
    testM("flatMap") {
      val fn = Gen.function[Random with Sized, Int, Chunk[Int]](smallChunks(intGen))
      check(smallChunks(intGen), fn) { (c, f) =>
        assert(c.flatMap(f).toList == c.toList.flatMap(f.andThen(_.toList)))
      }
    },
    testM("headOption") {
      check(mediumChunks(intGen))(c => assert(c.headOption == c.toList.headOption))
    },
    testM("lastOption") {
      check(mediumChunks(intGen))(c => assert(c.lastOption == c.toList.lastOption))
    },
    testM("indexWhere") {
    ...
    testM("find") {
      val fn = Gen.function[Random with Sized, Int, Boolean](Gen.boolean)
      check(mediumChunks(intGen), fn)((chunk, p) => assert(chunk.find(p) == chunk.toList.find(p)))
    },
    testM("filter") {
      val fn = Gen.function[Random with Sized, Int, Boolean](Gen.boolean)
      check(mediumChunks(intGen), fn)((chunk, p) => assert(chunk.filter(p).toList == chunk.toList.filter(p)))
    },
    suite("filterM")(
      testM("filterM happy path")(checkM(mediumChunks(intGen), Gen.function(Gen.boolean)) { (chunk, p) =>
        chunk.filterM(s => UIO.succeed(p(s))).map(a => assert(a == chunk.filter(p)))
      }),
      testM("filterM error") {
        Chunk(1, 2, 3).filterM(_ => IO.fail("Ouch")).either.map(a => assert(a == Left("Ouch")))
      } @@ zioTag(errors)
    ),
    testM("drop chunk") {
      check(largeChunks(intGen), intGen)((chunk, n) => assert(chunk.drop(n).toList == chunk.toList.drop(n)))
    },
    testM("take chunk") {
      check(chunkWithIndex(Gen.unit)) { case (c, n) =>
        assert(c.take(n).toList == c.toList.take(n))
      }
    },
    testM("dropWhile chunk") {
      check(mediumChunks(intGen), toBoolFn[Random, Int]) { (c, p) =>
        assert(c.dropWhile(p).toList == c.toList.dropWhile(p))
      }
    },
    suite("dropWhileM")(
    ...
    testM("takeWhile chunk") {
      check(mediumChunks(intGen), toBoolFn[Random, Int]) { (c, p) =>
        assert(c.takeWhile(p).toList == c.toList.takeWhile(p))
      }
    },
    suite("takeWhileM")(
    ...
    testM("toArray") {
      check(mediumChunks(Gen.alphaNumericString))(c => assert(c.toArray.toList == c.toList))
    },
    ...
    test("toArray for an empty Chunk of type String") {
      assert(Chunk.empty.toArray[String] sameElements Array.empty[String])
    },
    test("to Array for an empty Chunk using filter") {
      assert(Chunk(1).filter(_ == 2).map(_.toString).toArray[String] sameElements Array.empty[String])
    },
    ...
    test("toArray for a Chunk of any type") {
      ...
      assert(Chunk.fromIterable(v).toArray.toVector == v)
    },
    suite("collect")(
      ...
      testM("collect chunk") {
        ...
        check(mediumChunks(intGen), pfGen)((c, pf) => assert(c.collect(pf).toList == c.toList.collect(pf)))
      }
    ),
    suite("collectM")(
    ...
    suite("collectWhile")(
      ...
      testM("collectWhile chunk") {
        ...
        check(mediumChunks(intGen), pfGen) { (c, pf) =>
          assert(c.collectWhile(pf).toList == c.toList.takeWhile(pf.isDefinedAt).map(pf.apply))
        }
      }
    ),
    suite("collectWhileM")(
    ...
    testM("foreach") {
      check(mediumChunks(intGen)) { c =>
        ...
        assert(sum == c.toList.sum)
      }
    },
    testM("concat chunk") {
      check(smallChunks(intGen), smallChunks(intGen)) { (c1, c2) =>
        assert((c1 ++ c2).toList == c1.toList ++ c2.toList)
      }
    },
    ...
    test("nullArrayBug") {
      ...
      assert(c.filter(_ => false).map(_ * 2).isEmpty)
    },
    ...
    test("toArrayOnConcatOfEmptyAndInts") {
      assert(Chunk.empty ++ Chunk.fromArray(Array(1, 2, 3)) == Chunk(1, 2, 3))
    },
    ...
    test("zipAll") {
      ...
      assert(a.zipAll(a) == c, a.zipAll(b) == d, b.zipAll(a) == e)
    },
    test("zipAllWith") {
      assert(
    ...
    test("zipWithIndex") {
      ...
      val ch         = ch1 ++ ch2
      assert(ch.zipWithIndex.toList == ch.toList.zipWithIndex)
    },
    testM("zipWithIndex on concatenated chunks") {
      check(smallChunks(intGen), smallChunks(intGen)) { (c1, c2) =>
        ...
        assert(items.toList == c1.toList ++ c2.toList, indices.toList == (0 until (c1.size + c2.size)).toList)
      }
    },
    ...
    testM("chunks can be constructed from heterogeneous collections") {
      check(Gen.listOf(Gen.oneOf(Gen.anyInt, Gen.anyString, Gen.none))) { as =>
        assert(Chunk.fromIterable(as).toList == as)
      }
    },
    test("unfold") {
      assert(
    ...
    testM("split") {
      ...
      check(smallInts, chunks) { (n, chunk) =>
        ...
        assert(groups.flatten == chunk, groups.size == (n min chunk.length))
      }
    },
    testM("fromIterator") {
      check(Gen.chunkOf(Gen.anyInt)) { as =>
        assert(Chunk.fromIterator(as.iterator) == as)
      }
    }
  )
}
