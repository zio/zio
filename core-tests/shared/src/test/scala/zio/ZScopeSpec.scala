...
object ZScopeSpec extends ZIOBaseSpec {
  ...
  def scopeTest[A](label: String, a: A)(f: (Ref[A], ZScope[Unit]) => UIO[A]) =
    testM(label) {
      for {
        ref      <- Ref.make[A](a)
  ...
  def spec = suite("ZScopeSpec")(
    ...
    scopeTest("one finalizer", 0)((ref, scope) => scope.ensure(_ => ref.update(_ + 1)) as 1),
    scopeTest("two finalizers", List.empty[String]) { (ref, scope) =>
      scope.ensure(_ => ref.update("foo" :: _)) *>
        scope.ensure(_ => ref.update("bar" :: _)) as (List("bar", "foo"))
    }
  )
}
