    ...
    suite("collectFirst")(
      ...
      testM("is atomic") {
        for {
          ...
          _      <- STM.foreach((0 until N).toList)(i => tArray.update(i, _ => Some(1))).commit
    ...
    suite("find")(
      ...
      testM("is atomic") {
        for {
          ...
          _         <- STM.foreach((0 until N).toList)(i => tArray.update(i, _ => 1)).commit
          result    <- findFiber.join
    ...
    suite("transform")(
      testM("updates values atomically") {
        for {
          ...
          _              <- STM.foreach((0 until N).toList)(idx => tArray.update(idx, _ + "+c")).commit
