...
object ZQueueSpec extends ZIOBaseSpec {
  ...
  def spec = suite("ZQueueSpec")(
    ...
    testM("reactive queue backpressure pattern") {
      ...
      val program4: Task[Unit] = ZIO.effect(Thread.sleep(1000))
      val program5: Task[Unit] = ZIO.effect(Thread.sleep(1000))
      val program6: Task[Unit] = ZIO.effect { promise.success("this program it should never being processed"); () }

      for {
        _ <- queue.offer(program1)
        _ <- queue.offer(program2)
        _ <- queue.offer(program3)
        _ <- queue.offer(program4)
