...
object ScheduleSpec extends ZIOBaseSpec {
  ...
  def spec: ZSpec[Environment, Failure] = suite("ScheduleSpec")(
    /**
     * Retry `once` means that we try to exec `io`, get and error, try again to exec `io`, and whatever the output is,
     * we return that second result. The three following tests test retry when:
  ...
  /**
   * A function that increments ref each time it is called. It always fails, with the incremented value in error
   */
  ...
  /**
   * A function that increments ref each time it is called. It returns either a failure if ref value is 0 or less before
   * increment, and the value in other cases.
   */
