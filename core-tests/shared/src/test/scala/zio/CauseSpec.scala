...
object CauseSpec extends ZIOBaseSpec {
  ...
  def spec = suite("CauseSpec")(
    suite("Cause")(
      ...
      test("failureOrCause: defect wins over failure in Both") {
        ...
        assert(combined.failureOrCause)(isRight(equalTo(defect)))
      },
  ...
  suite("failureOrCause — property-based (defect always wins)")(
    test("forAll: Cause with any Die always returns Right from failureOrCause") {
      check(Gen.string) { msg =>
        ...
        assertTrue(both.failureOrCause.isRight) &&
        assertTrue(both.failureOrCause.toOption.get.isDie)
      }
    },
    test("forAll: Cause with any Interrupt always returns Right from failureOrCause") {
      check(Gen.string) { _ =>
        ...
        assertTrue(both.failureOrCause.isRight) &&
        assertTrue(both.failureOrCause.toOption.get.isInterrupted)
      }
    },
    ...
    test("forAll: Die has higher priority than any number of failures") {
      check(Gen.listOf(Gen.string).map(_.take(5))) { msgs =>
        ...
        assertTrue(withDie.failureOrCause.isRight)
      }
    }
  )
  ...
  val fiberIds: Gen[Any, FiberId] =
    Gen.int.zipWith(Gen.int)(FiberId(_, _, Trace.empty)
      test("failureOrCause returns full self cause (not stripped) when Die and Fail coexist") {
        val dieCause  = Cause.die(new RuntimeException("boom"))
        val failCause = Cause.fail("err")
        val combined  = dieCause && failCause
        val result    = combined.failureOrCause
        assertTrue(result.isRight) &&
        assertTrue(result.toOption.get == combined)
      },
      test("failureTraceOrCause returns full self cause when Interrupt and Fail coexist") {
        val interruptCause = Cause.interrupt(zio.FiberId.None)
        val failCause      = Cause.fail("err")
        val combined       = interruptCause && failCause
        val result         = combined.failureTraceOrCause
        assertTrue(result.isRight) &&
        assertTrue(result.toOption.get == combined)
      },
      test("failureOrCause returns full self when Then(Die, Fail)") {
        val combined = Cause.die(new RuntimeException("boom")) ++ Cause.fail("err")
        val result   = combined.failureOrCause
        assertTrue(result.isRight) &&
        assertTrue(result.toOption.get == combined)
      },
