  ...
  def spec =
    suite("TagCorrectnessSpec")(
      ...
      test("Issue #4809") {
        ...
        object Service {
          val live: UProvider[Service[Int]] =
            ZProvider.succeed {
              new Service[Int] {
                def foo(t: Int) =
                  ZIO.succeed(t)
      ...
      test("Issue #4564") {
        trait Svc[A]
        def testBaseLayer[R, A: Tag]: ZProvider[R, Nothing, Svc[A]] =
          ZIO.environmentWith[R](_ => new Svc[A] {}).toProvider[Svc[A]]
        def testSecondLayer[A: Tag]: ZProvider[Svc[A], Nothing, Svc[A]] =
          ZProvider.fromFunction[Svc[A], Svc[A]] { environment =>
            environment.get
          }
      ...
      test("Issue #3816") {
        ...
        object ContainerProvider {
          ...
          def layer[A: Tag, D <: Container[A]: Tag](container: D): UProvider[ContainerProvider[A, D]] =
            ZProvider.succeed {
              new Service[A, D] {
...
object HigherKindedTagCorrectness extends DefaultRunnableSpec {
  ...
  object Cache {
    def live[F[_], K, V](
      f: Option[V] => F[V]
    )(implicit tag: Tag[Cache[F, K, V]]): ZProvider[Any, Nothing, Cache[F, K, V]] =
      ZProvider {
        for {
  ...
  val myCache: ZProvider[Any, Nothing, Cache[Option, Int, String]] =
