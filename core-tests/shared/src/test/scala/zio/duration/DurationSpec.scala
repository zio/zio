...
object DurationSpec
    extends ZIOBaseSpec(
      suite("DurationSpec")(
        ...
        test(
          "Make a Java stdlib j.t.Duration and check that: A nano-adjusted Long.MaxValue second j.t.Duration converts to Infinity"
        ) {
          assert(Duration.fromJava(JavaDuration.ofSeconds(Long.MaxValue, 1L)), equalTo(Duration.Infinity: Duration))
        },
        test(
          "Make a Java stdlib j.t.Duration and check that: A j.t.Duration constructed from Infinity converts to Infinity"
        ) {
          assert(Duration.fromJava(Duration.Infinity.asJava), equalTo(Duration.Infinity: Duration))
        },
        test(
          "Make a Java stdlib j.t.Duration and check that: A Long.MaxValue - 1 second j.t.Duration converts to Infinity"
        ) {
          assert(Duration.fromJava(JavaDuration.ofSeconds(Long.MaxValue - 1)), equalTo(Duration.Infinity: Duration))
        },
        test(
          "Make a Java stdlib j.t.Duration and check that: A +ve j.t.Duration whose nano conversion overflows converts to Infinity"
        ) {
          assert(
        ...
        test(
          "Make a Java stdlib j.t.Duration and check that: A -ve j.t.Duration whose nano conversion overflows converts to Zero"
        ) {
          assert(
