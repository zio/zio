    ...
    suite("ZPoolSpec") {
      test("preallocates pool items") {
        for {
          ...
          scope <- Scope.make
          _     <- ZPool.make(get, 10).provideService(scope)
        ...
        test("blocks when item not available") {
          for {
            ...
            pool   <- ZPool.make(get, 10).provideService(scope)
            _      <- count.get.repeatUntil(_ == 10)
            _      <- ZIO.collectAll(List.fill(10)(pool.get.provideService(scope)))
        ...
        test("max pool size") {
          for {
            ...
            scope   <- Scope.make
            pool <-
              ZPool.make(get, 10 to 15, 60.seconds).provideSomeEnvironment[Clock](_.union[Scope](ZEnvironment(scope)))
