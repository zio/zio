  ...
  def spec: Spec[Any, TestFailure[Any], TestSuccess] =
    suite("ParallelMockSpec")(
      test("Count calls for the same expectation") {
        ...
        val app  = ZIO.collectAllPar(Vector.fill(100)(ImpureModule.singleParam(1))).provide(mock)
      ...
      test("Collect calls for all expectations") {
        ...
        val app = ZIO.collectAllPar(params.map(i => ImpureModule.singleParam(i))).provide(mock)
