...
import zio.test.environment.{ liveEnvironment, testEnvironmentManaged, TestEnvironment }
import zio.{ UIO, ZIO }

object TestUtils {

  def execute[E, L, S](spec: ZSpec[TestEnvironment, E, L, S]): UIO[ExecutedSpec[E, L, S]] =
    TestExecutor.managed(liveEnvironment, testEnvironmentManaged).run(spec, ExecutionStrategy.Sequential)
