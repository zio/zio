    ...
    suite("ConsoleTestOutputSpec")(
      suite("reports")(
        ...
        test("failed test suite") {
          runLog(suite2).map(res => suite2Streaming.map(expected => containsUnstyled(res, expected)).reduce(_ && _))
        },
        test("multiple test suites") {
          runLog(suite3).map(res => suite2Streaming.map(expected => containsUnstyled(res, expected)).reduce(_ && _))
        },
