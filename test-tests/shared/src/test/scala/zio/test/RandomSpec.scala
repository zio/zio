...
object RandomSpec extends ZIOBaseSpec {
  ...
  def spec = suite("RandomSpec")(
    ...
    test("referential transparency") {
      ...
      ZIO
        .runtime[Any]
        .map { rt =>
          val x = Unsafe.unsafeCompat(implicit u => rt.unsafeRun(test.flatMap[Any, Nothing, Int](_.nextInt)))
          val y = Unsafe.unsafeCompat(implicit u => rt.unsafeRun(test.flatMap[Any, Nothing, Int](_.nextInt)))
          assert(x)(equalTo(y))
        }
    },
  ...
  )(extract: ZRandom => UIO[A]): URIO[TestConfig, TestResult] =
    check(Gen.long) { seed =>
      for {
  ...
  def forAllEqual[A](
    f: ZRandom => UIO[A]
  )(g: SRandom => A): URIO[TestConfig, TestResult] =
    check(Gen.long) { seed =>
      for {
        sRandom    <- ZIO.succeed(new SRandom(seed))
        testRandom <- TestRandom.makeTest(DefaultData)
  ...
  def forAllEqualBytes: URIO[TestConfig, TestResult] =
    check(Gen.long) { seed =>
      for {
        sRandom    <- ZIO.succeed(new SRandom(seed))
        testRandom <- TestRandom.makeTest(DefaultData)
  ...
  def forAllEqualGaussian: URIO[TestConfig, TestResult] =
    check(Gen.long) { seed =>
      for {
        sRandom    <- ZIO.succeed(new SRandom(seed))
        testRandom <- TestRandom.makeTest(DefaultData)
  ...
  def forAllEqualN[A](
    f: (ZRandom, Int) => UIO[A]
  )(g: (SRandom, Int) => A): URIO[TestConfig, TestResult] =
  ...
  )(g: (SRandom, List[Int]) => List[Int]): ZIO[Sized with TestConfig, Nothing, TestResult] =
