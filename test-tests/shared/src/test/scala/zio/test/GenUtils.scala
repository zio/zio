...
object GenUtils {
  ...
  def checkShrink[A](gen: Gen[Any, A])(a: A): UIO[TestResult] =
    provideSize(alwaysShrinksTo(gen)(a: A))(shrinkSize)
  ...
  def shrinks[R, A](gen: Gen[R, A]): ZIO[R, Nothing, List[A]] =
    gen.sample.forever.take(1).flatMap(_.shrinkSearch(_ => true)).take(shrinkSize * 10L).runCollect.map(_.toList)
  ...
  def sample100[R, A](gen: Gen[R, A]): ZIO[R, Nothing, List[A]] =
    gen.sample.map(_.value).forever.take(shrinkSize.toLong).runCollect.map(_.toList)
  ...
  def shrink[R, A](gen: Gen[R, A]): URIO[R, A] =
    gen.sample.take(1).flatMap(_.shrinkSearch(_ => true)).take(shrinkSize * 10L).runLast.map(_.get)
  ...
  def shrinkWith[R, A](gen: Gen[R, A])(f: A => Boolean): ZIO[R, Nothing, List[A]] =
    gen.sample.take(1).flatMap(_.shrinkSearch(!f(_))).take(shrinkSize * 10L).filter(!f(_)).runCollect.map(_.toList)
