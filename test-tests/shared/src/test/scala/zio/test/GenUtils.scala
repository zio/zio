...
import zio.{Exit, Has, Random, UIO, URIO, ZIO}
...
object GenUtils {
  ...
  def equalShrink[A](left: Gen[Has[Random], A], right: Gen[Has[Random], A]): UIO[Boolean] = {
    ...
    for {
      leftShrinks  <- ZIO.collectAll(List.fill(100)(shrinks(left))).provideServices(testRandom)
      rightShrinks <- ZIO.collectAll(List.fill(100)(shrinks(right))).provideServices(testRandom)
  ...
  def equalSample[A](left: Gen[Has[Random], A], right: Gen[Has[Random], A]): UIO[Boolean] = {
    val testRandom = TestRandom.deterministic
    for {
      leftSample  <- sample100(left).provideServices(testRandom)
      rightSample <- sample100(right).provideServices(testRandom)
  ...
  def provideSize[A](zio: ZIO[Has[Random] with Has[Sized], Nothing, A])(n: Int): URIO[Has[Random], A] =
    zio.provideServices[Nothing, Has[Random], Has[Random] with Has[Sized]](Random.any ++ Sized.live(n))
