...
object GenUtils {
  ...
  def equalShrink[A](left: Gen[Random, A], right: Gen[Random, A]): UIO[Boolean] = {
    ...
    for {
      leftShrinks  <- ZIO.collectAll(List.fill(100)(shrinks(left))).manuallyProvide(testRandom)
      rightShrinks <- ZIO.collectAll(List.fill(100)(shrinks(right))).manuallyProvide(testRandom)
  ...
  def equalSample[A](left: Gen[Random, A], right: Gen[Random, A]): UIO[Boolean] = {
    val testRandom = TestRandom.deterministic
    for {
      leftSample  <- sample100(left).manuallyProvide(testRandom)
      rightSample <- sample100(right).manuallyProvide(testRandom)
  ...
  def provideSize[A](zio: ZIO[Random with Sized, Nothing, A])(n: Int): URIO[Random, A] =
    zio.manuallyProvide[Nothing, Random, Random with Sized](Random.any ++ Sized.live(n))
