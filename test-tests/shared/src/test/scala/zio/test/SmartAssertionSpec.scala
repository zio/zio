...
object SmartAssertionSpec extends ZIOBaseSpec {
  ...
  def spec: ZSpec[Environment, Failure] =
    suite("SmartAssertionSpec")(
      suite("user-defined smart assertions")(
        ...
        test("custom assertions") {
          val thing: Color = Blue("blue")
          assertTrue(Color.isRed(thing))
        },
        test("reports source location of actual usage") {
          propagatesSourceLocation("hello")
        }
      ) @@ failing,
      suite("Array")(
        suite("==")(
  ...
  // actual location.
  def propagatesSourceLocation(string: String)(implicit sourceLocation: SourceLocation): Assert =
  ...
  object Color {
    ...
    def isRed(color: Color): SmartAssertion[Boolean] = SmartAssertion.fromEither {
      color match {
        case Red(_) => Right(true)
        case _      => Left(s"$color IS NOT NICE!")
      }
    }
  }
