...
//  }
  ...
  def snapshotTest[Any, E >: Throwable](
    ...
    println(s"SOURCE LOCATION ${sourceLocation}")
  ...
  def snapshotTestM[R, E >: Throwable](
    ...
    println(s"SOURCE LOCATION: $sourceLocation")
    val path = Path.of(sourceLocation.path)
    println("PARENT", Path.of(path.getParent.toString, "SnapshotSpec_snapshots", label))
    ...
    testM(label)(
      for {
        ...
        res <- if (getClass.getResource(snapFileName) eq null)
                 ...
                 ZIO
                   .bracket[R, E, scala.io.Source](
                     Task(scala.io.Source.fromInputStream(getClass.getResourceAsStream(snapFileName)))
                   )((s: scala.io.Source) => UIO(s.close()))((s: scala.io.Source) => UIO(s.getLines.mkString("\n")))
                   .map { (snapshot: String) =>
//                     assertTrue(snapshot == actual): TestResult
                     CompileVariants.assertProxy(actual, snapshot, label)(equalToSnapshot(snapshot))
                   }
      } yield res
  ...
  override def spec: ZSpec[Any, Any] =
    suite("matchSnapshot suite")(
      snapshotTest("firsta")("goodbye how are you doing on this terrible day?"),
