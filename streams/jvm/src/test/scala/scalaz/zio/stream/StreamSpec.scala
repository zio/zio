...
class StreamSpec(implicit ee: org.specs2.concurrent.ExecutionEnv)
  ...
  private def take0ShortCircuitsStreamNever =
    unsafeRun(
      for {
        units <- Stream.never.take(0).run(Sink.accum[Unit])
  ...
  private def take1ShortCircuitsStreamNever =
    unsafeRun(
      for {
        ints <- (Stream(1) ++ Stream.never).take(1).run(Sink.accum[Int])
  ...
  private def transduce = {
    ...
    val parser     = ZSink.accumWhile[Char](_.isDigit).map(_.mkString.toInt) <* ZSink.accumWhile[Char](_ == ',')
  ...
  private def bracket =
    unsafeRun(
      for {
        ...
        result         <- iteratorStream.run(Sink.accum[Int])
        released       <- done.get
  ...
  private def unTake =
    unsafeRun(
      Stream
        ...
        .use { q =>
          Stream.fromQueue(q).unTake.run(Sink.accum[Int])
        }
  ...
  private def fromInputStream = unsafeRun {
    ...
    ZStream.fromInputStream(is).run(Sink.accum[Chunk[Byte]]) map { chunks =>
