...
abstract class ZStream[-R, +E, +O](val process: ZManaged[R, Nothing, ZIO[R, Option[E], Chunk[O]]])
  ...
  final def combine[R1 <: R, E1 >: E, S, O2, O3](that: ZStream[R1, E1, O2])(s: S)(
    ...
    ZStream.make[R1, E1, O3] {
      for {
        ...
        pull <- ZStream
                  .unfoldZIO(s)(s => f(s, left.pullElement, right.pullElement).flatMap(ZIO.done(_).optional))
                  .process
      } yield pull
    }
  ...
  final def combineChunks[R1 <: R, E1 >: E, S, O2, O3](that: ZStream[R1, E1, O2])(s: S)(
    ...
    ZStream.make[R1, E1, O3] {
      for {
        ...
        right <- that.process
        pull <- ZStream
                  .unfoldChunkZIO(s)(s => f(s, left, right).flatMap(ZIO.done(_).optional))
                  .process
      } yield pull
    }
  ...
  final def ensuring[R1 <: R](fin: ZIO[R1, Nothing, Any]): ZStream[R1, E, O] =
    ZStream.make(self.process.ensuring(fin))

  /**
   * Executes the provided finalizer before this stream's finalizers run.
   */
  final def ensuringFirst[R1 <: R](fin: ZIO[R1, Nothing, Any]): ZStream[R1, E, O] =
    ZStream.make(self.process.ensuringFirst(fin))
          ...
          _ <- self.foreachManaged { a =>
                 for {
                   ...
                   innerStream = ZStream
                                   ...
                                   .foldCauseZIO(
                                     cause => out.offer(Pull.failCause(cause)) *> innerFailure.fail(cause).unit,
                                     _ => ZIO.unit
                                   )
                   _ <- innerStream.fork
                 ...
                 cause =>
                   (getChildren.flatMap(Fiber.interruptAll(_)) *> out.offer(Pull.failCause(cause)).unit).toManaged,
                 _ =>
  ...
  def mapChunksZIO[R1 <: R, E1 >: E, O2](f: Chunk[O] => ZIO[R1, E1, Chunk[O2]]): ZStream[R1, E1, O2] =
    ZStream.make(self.process.map(_.flatMap(f(_).mapError(Some(_)))))
  ...
  def mapError[E2](f: E => E2): ZStream[R, E2, O] =
    ZStream.make(self.process.map(_.mapError(_.map(f))))
  ...
  final def provide(r: R)(implicit ev: NeedsEnv[R]): ZStream[Any, E, O] =
    ZStream.make(self.process.provide(r).map(_.provide(r)))
  ...
  final def repeatWith[R1 <: R, B, C](
    ...
    ZStream.make[R1 with Has[Clock], E, C] {
      for {
        sdriver    <- schedule.driver.toManaged
  ...
  final def scheduleWith[R1 <: R, E1 >: E, B, C](
    ...
    ZStream.make[R1 with Has[Clock], E1, C] {
      for {
        as     <- self.process.mapZIO(BufferedPull.make(_))
        driver <- schedule.driver.toManaged
  ...
  final def debounce[E1 >: E, O2 >: O](d: Duration): ZStream[R with Has[Clock], E1, O2] = {
    ...
    ZStream.make[R with Has[Clock], E1, O2] {
      for {
        chunks <- self.process
  ...
  final def timeout(d: Duration): ZStream[R with Has[Clock], E, O] =
    ZStream.make[R with Has[Clock], E, O] {
      for {
  ...
  final def timeoutErrorCause[E1 >: E](cause: Cause[E1])(d: Duration): ZStream[R with Has[Clock], E1, O] =
    ZStream.make[R with Has[Clock], E1, O] {
...
object ZStream extends ZStreamPlatformSpecificConstructors {
  ...
  val empty: ZStream[Any, Nothing, Nothing] =
    ZStream.make(ZManaged.succeedNow(Pull.end))
  ...
  def iterate[A](a: A)(f: A => A): ZStream[Any, Nothing, A] =
    ZStream.make(Ref.make(a).toManaged.map(_.getAndUpdate(f).map(Chunk.single(_))))
  ...
  val never: ZStream[Any, Nothing, Nothing] =
    ZStream.make(ZManaged.succeedNow(UIO.never))
  ...
  def range(min: Int, max: Int, chunkSize: Int = DefaultChunkSize): ZStream[Any, Nothing, Int] = {
    ...
    ZStream.make(Ref.makeManaged(min).map(pull))
  }
  ...
  object ZStreamConstructor extends ZStreamConstructorLowPriority1 {
     ...
     */
    implicit def JavaIteratorConstructor[A]: WithOut[ju.Iterator[A], ZStream[Any, Throwable, A]] =
      new ZStreamConstructor[ju.Iterator[A]] {
        type Out = ZStream[Any, Throwable, A]
        def make(input: => ju.Iterator[A]): ZStream[Any, Throwable, A] =
    ...
    implicit def JavaIteratorManagedConstructor[R, A]
      : WithOut[ZManaged[R, Throwable, ju.Iterator[A]], ZStream[R, Throwable, A]] =
      new ZStreamConstructor[ZManaged[R, Throwable, ju.Iterator[A]]] {
        type Out = ZStream[R, Throwable, A]
        def make(input: => ZManaged[R, Throwable, ju.Iterator[A]]): ZStream[R, Throwable, A] =
    ...
    implicit def JavaIteratorZIOConstructor[R, A]
      : WithOut[ZIO[R, Throwable, ju.Iterator[A]], ZStream[R, Throwable, A]] =
      new ZStreamConstructor[ZIO[R, Throwable, ju.Iterator[A]]] {
        type Out = ZStream[R, Throwable, A]
        def make(input: => ZIO[R, Throwable, ju.Iterator[A]]): ZStream[R, Throwable, A] =
