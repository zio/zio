...
class ZStream[-R, +E, +A](val process: ZManaged[R, E, Pull[R, E, A]]) extends Serializable { self =>
   ...
   */
  final def schedule[R1 <: R, A1 >: A](schedule: ZSchedule[R1, A, _]): ZStream[R1 with Clock, E, A1] =
   ...
   */
  final def scheduleElements[R1 <: R, A1 >: A](schedule: ZSchedule[R1, A, _]): ZStream[R1 with Clock, E, A1] =
