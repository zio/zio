...
final class ZStream[-R, +E, +A] private (val channel: ZChannel[R, Any, Any, Any, E, Chunk[A], Any]) { self =>
          ...
          lazy val process: ZChannel[Any, Any, Any, Any, E, Chunk[A], Unit] =
            ZChannel.fromZIO {
              ...
              exit.foldExit(
                Cause
                  .flipCauseOption(_)
                  .fold[ZChannel[Any, Any, Any, Any, E, Chunk[A], Unit]](ZChannel.unit)(ZChannel.refailCauseChannelFn),
  ...
  def find(f: A => Boolean)(implicit trace: Trace): ZStream[R, E, A] = {
    lazy val loop: ZChannel[R, E, Chunk[A], Any, E, Chunk[A], Any] =
      ZChannel.readInputCauseUnit((in: Chunk[A]) => in.find(f).fold(loop)(i => ZChannel.write(Chunk.single(i))))
  ...
  def findZIO[R1 <: R, E1 >: E, S](
    ...
    lazy val loop: ZChannel[R1, E, Chunk[A], Any, E1, Chunk[A], Any] =
      ZChannel.readInputCauseUnit((in: Chunk[A]) =>
        ZChannel.unwrap(in.findZIO(f).map(_.fold(loop)(i => ZChannel.write(Chunk.single(i)))))
      )
  ...
  def flattenChunks[A1](implicit ev: A <:< Chunk[A1], trace: Trace): ZStream[R, E, A1] = {
    ...
    lazy val flatten: ZChannel[Any, E, Chunk[Chunk[A1]], Any, E, Chunk[A1], Any] =
      ZChannel.readInputCauseUnit(chunks => ZChannel.writeChunk(chunks) *> flatten)
  ...
  def flattenExitOption[E1 >: E, A1](implicit
    ...
    lazy val process: ZChannel[R, E, Chunk[Exit[Option[E1], A1]], Any, E1, Chunk[A1], Any] =
      ZChannel.readInputCauseUnit(chunk => processChunk(chunk, process))
    ...
    def writer(fiber: Fiber[E1, Any]): ZChannel[R1, E1, Chunk[A], Any, E1, Chunk[A], Unit] =
      ZChannel.unwrap {
        fiber.poll.map {
          ...
          case Some(exit) =>
            exit.foldExit(ZChannel.refailCauseChannelFn, ZChannel.unitChannelFn)
        }
      }
  ...
  def repeatElementsWith[R1 <: R, E1 >: E, B, C](
    ...
    self.channel >>> {
      ...
      def feed(in: Chunk[A]): ZChannel[R1, E1, Chunk[A], Any, E1, Chunk[C], Any] =
        if (in.isEmpty) loop
        else {
          val a = in(0)
      ...
      def step(in: Chunk[A], a: A): ZChannel[R1, E1, Chunk[A], Any, E1, Chunk[C], Any] = {
        val advance = driver.next(a).as(ZChannel.write(Chunk.single(f(a))) *> step(in, a))
        val reset: ZIO[R1, Nothing, ZChannel[R1, E1, Chunk[A], Any, E1, Chunk[C], Any]] =
          for {
            b <- driver.last.orDie
            _ <- driver.reset
  ...
  def scheduleWith[R1 <: R, E1 >: E, B, C](
    ...
    def loop(
      ...
      if (chunkIterator.hasNextAt(index))
        ...
        ZChannel.readInputCauseUnit(chunk => loop(driver, chunk.chunkIterator, 0))
...
object ZStream extends ZStreamPlatformSpecificConstructors {
  ...
  def absolve[R, E, O](xs: ZStream[R, E, Either[E, O]])(implicit trace: Trace): ZStream[R, E, O] = {
    lazy val loop: ZChannel[Any, E, Chunk[Either[E, O]], Any, E, Chunk[O], Any] = ZChannel.readInputCauseUnit {
      (in: Chunk[Either[E, O]]) =>
  ...
  def fromChunkHubWithShutdown[O](hub: => Hub[Chunk[O]])(implicit
    ...
    ZStream.suspend {
      val hub0 = hub
      fromChunkHub(hub0).ensuring(hub0.shutdown)
    }
  ...
  def fromChunkHubScopedWithShutdown[O](
    ...
    ZIO.suspendSucceed {
      val hub0 = hub
      fromChunkHubScoped(hub0).map(_.ensuring(hub0.shutdown))
    }
  ...
  def fromChunkQueue[O](
    ...
    repeatZIOChunkOption {
      ...
      queue0.take
        .catchAllCause(c =>
          queue0.isShutdown.flatMap { down =>
  ...
  def fromChunkQueueWithShutdown[O](queue: => Dequeue[Chunk[O]])(implicit
    ...
    ZStream.suspend {
      val queue0 = queue
      fromChunkQueue(queue0).ensuring(queue0.shutdown)
    }
