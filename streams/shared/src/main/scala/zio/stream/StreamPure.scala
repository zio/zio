...
private[stream] trait StreamPure[+A] extends ZStream[Any, Nothing, A] { self =>
  ...
  /**
   * For a StreamPure foldMapError is implemented in terms of fold.
   * fold is just forwarding to fold0, which is used to prevent infinite loops.
   */
  protected def fold0[R, E, A1 >: A, S]: Fold[R, E, A1, S]
  ...
  final def foldMapError[R, E, A1 >: A, S](e: Nothing => E): Fold[R, E, A1, S] =
    fold0[R, E, A1, S]
  ...
  override def map[B](f0: A => B): StreamPure[B] = new StreamPure[B] {
    def foldPureLazy[B1 >: B, S](s: S)(cont: S => Boolean)(f: (S, B1) => S): S =
      self.foldPureLazy[A, S](s)(cont)((s, a) => f(s, f0(a)))

    def fold0[R, E, B1 >: B, S]: Fold[R, E, B1, S] =
  ...
  override def zipWithIndex: StreamPure[(A, Int)] = new StreamPure[(A, Int)] {
    def foldPureLazy[A1 >: (A, Int), S](s: S)(cont: S => Boolean)(f: (S, A1) => S): S =
      self
    ...
    def fold0[R, E, A1 >: (A, Int), S]: Fold[R, E, A1, S] =
...
private[stream] object StreamPure extends Serializable {
  ...
  final val empty: StreamPure[Nothing] = new StreamPure[Nothing] {
    def foldPureLazy[A, S](s: S)(cont: S => Boolean)(f: (S, A) => S): S = s

    def fold0[R1, E1, A1, S]: ZStream.Fold[R1, E1, A1, S] =
  ...
  final def fromIterable[A](it: Iterable[A]): StreamPure[A] = new StreamPure[A] {
    ...
    def fold0[R1, E1, A1 >: A, S]: ZStream.Fold[R1, E1, A1, S] =
