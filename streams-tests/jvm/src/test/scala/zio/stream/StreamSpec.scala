...
class ZStreamSpec(implicit ee: org.specs2.concurrent.ExecutionEnv)
  ...
  private def catchAll = {
    val e = "boom"
    unsafeRunSync(
      (Stream.range(0, 2) ++ Stream.fail(e))
        .catchAll(e => Stream.succeed(e.length))
        .run(Sink.collectAll[Int])
    ) must_== Success(List(0, 1, 2, 4))
  }
