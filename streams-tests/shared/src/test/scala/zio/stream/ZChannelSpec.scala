        ...
        test("simple provide") {
          assertM(
            ZChannel
              ...
              .provideAll(ZEnvironment(100))
              .run
          )(equalTo(100))
        },
        test("provide <*> provide") {
          assertM(
            (ZChannel.fromZIO(ZIO.service[Int]).provideAll(ZEnvironment(100)) <*>
              ZChannel.fromZIO(ZIO.service[Int]).provideAll(ZEnvironment(200))).run
          )(equalTo((100, 200)))
        },
        ...
        test("provide is modular") {
          assertM(
            (for {
              ...
              v2 <- ZChannel.fromZIO(ZIO.service[Int]).provideAll(ZEnvironment(2))
              v3 <- ZChannel.fromZIO(ZIO.service[Int])
            } yield (v1, v2, v3)).runDrain.provideAll(ZEnvironment(4))
          )(equalTo((4, 2, 4)))
        }
      )
    )
