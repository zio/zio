...
Let's create a `ZProvider` for `OpenTracing` which provides us Jaeger tracer. Each microservice uses this dependency to send its tracing data to the _Jaeger Backend_:
...
object JaegerTracer {
  def makeJaegerTracer(host: String, serviceName: String): ZProvider[Clock, Throwable, Clock with OpenTracing] =
