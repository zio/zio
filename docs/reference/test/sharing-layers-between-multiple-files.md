...
:::note
When layers of the same type are defined at the class level within different test suites, they will be initialized separately for each test suite. This can lead to resource exhaustion, especially with limited resources like database connections or connection pools. To avoid this, ensure that such layers are defined centrally, such as in the bootstrap layer, to be shared properly across multiple test suites.
:::
