...
description: "ZIO's lightweight built-in logging facade with support for log levels, spans, and contextual annotations."
keywords:
  - "ZIO Logging"
  - "Log Levels"
  - "Logging Spans"
...
- **[ZLogger](../z-logger.md)** — Reference documentation for `ZLogger[-Message, +Output]`: the composable functional logging interface, its combinators, predefined instances, and `ZTestLogger` for in-memory test capture.
