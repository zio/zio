...
The `@mockable[A]` generates _capability tags_ and _mock provider_ into annotated object.
...
```scala
import zio.{ Has, UIO, URProvider, ZProvider }
...
object AccountObserverMock extends Mock[Has[AccountObserver.Service]] {
  ...
  val compose: URProvider[Has[Proxy], AccountObserver] =
    ZProvider.fromServiceM { proxy =>
      withRuntime.map { rts =>
