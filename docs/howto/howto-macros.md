...
object AccountObserver {
  ...
  def processEvent(event: AccountEvent) =
    ZIO.environmentWithZIO[Has[AccountObserver.Service]](_.get[Service].processEvent(event))
}
```
...
object Module {
  ...
  def impureMethod(v: Something): ZIO[Service, Throwable, SomethingElse] =
    ZIO.environmentWithZIO[Has[Module.Service]](s => ZIO(s.get[Service].impureMethod(v)))
}
```
