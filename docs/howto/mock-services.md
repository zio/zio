...
We'll be assuming you've read about modules and providers in the [contextual types][doc-contextual-types] guide. In the main sources we define the _service_, a module alias and _capability accessors_. In test sources we're defining the _mock object_ which extends `zio.test.mock.Mock` which holds _capability tags_ and _compose provider_.
...
object ExampleMock extends Mock[Example] {
  ...
  val compose: URProvider[Proxy, Example] = ???
}
```
...
Finally we need to define a _compose provider_ that can create our environment from a `Proxy`.
...
A reference to this provider is passed to _capability tags_ so it can be used to automatically build environment for composed expectations on
multiple services.
...
object AccountObserver {
  ...
  val live: ZProvider[Console, Nothing, AccountObserver] =
    { (console: Console) =>
      new Service {
        def processEvent(event: AccountEvent): UIO[Unit] =
          for {
...
object AccountObserverMock extends Mock[AccountObserver] {
  ...
  val compose: URProvider[Proxy, AccountObserver] =
...
object ExampleMock extends Mock[Example.Service] {
  object ZeroArgs  extends Effect[Unit, Nothing, Int]
  object SingleArg extends Effect[Int, Nothing, String]
  val compose: URProvider[Proxy, Example.Service] =
    ZIO.service[Proxy].map { proxy =>
      new Example.Service {
        def zeroArgs             = proxy(ZeroArgs)
        def singleArg(arg1: Int) = proxy(SingleArg, arg1)
...
Often the dependency on a collaborator is only in some branches of the code. To test the correct behaviour of branches without depedencies, we still have to provide it to the environment, but we would like to assert it was never called. With the `Mock.empty` method you can obtain a `ZProvider` with an empty service (no calls expected).
...
In some cases we have more than one collaborating service being called. You can create mocks for rich environments and as you enrich the environment by using _capability tags_ from another service, the underlying mocked provider will be updated.

```scala mdoc:silent
import zio.test.mock.MockRandom

val combinedEnv: UProvider[Console with Random] = (
...
object PolyExampleMock extends Mock[PolyExample] {
  ...
  val compose: URProvider[Proxy, PolyExample] =
    ZIO.serviceWithZIO[Proxy] { proxy =>
      withRuntime[Any].map { rts =>
