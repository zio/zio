...
The `ZIOApp` trait is an entry point for a ZIO application that allows sharing providers between applications. It also
...
The `<>` operator combines the providers of the two applications, composes their hooks, and then runs the two applications in parallel.
