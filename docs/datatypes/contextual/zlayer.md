...
### An Example of Writing Polymorphic Services

In ZIO we can also write polymorphic services. Assume we have the following service interface:
...
It's accessor methods would be like as below. We should put `izumi.reflect.Tag` as a context bound for `K`, `V` and `E` type parameters:
...
Note that if we don't use `izumi.reflect.Tag` the compiler error us as below:
...
Here is the in-memory implementation of this key-value store:
...
Now finally we can write application with this service:
