...
The idiomatic way to work with `ZState` is as part of the environment using operators defined on `ZIO`. So instead of creating `ZState` directly using `ZState.make` constructor, we can access the `ZState` from the environment, and finally, provide proper dependency using `ZState.makeDeps` constructor:
...
object ZStateExample extends zio.ZIOAppDefault {
  ...
  def run = myApp.injectCustom(ZState.makeDeps(0))
}
```
...
object ZStateExample extends zio.ZIOAppDefault {
  ...
  def run = myApp.injectCustom(ZState.makeDeps(MyState(0)))
}
```
