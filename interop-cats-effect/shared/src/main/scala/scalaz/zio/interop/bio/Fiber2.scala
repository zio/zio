...
abstract class Fiber2[F[+ _, + _], E, A] { self =>
   ...
   * with an error, if the joined fiber `self` succeeds the effect will return an `A` and
   * if the joining fiber or the `self` fiber are interrupted the returned effect will also
   * be interrupted.
   *
   * TODO: Example:
   * {{{
