...
import zio.internal.legacy.WeakConcurrentBag.IsAlive
...
private[zio] class WeakConcurrentBag[A <: AnyRef](nurserySize: Int, isAlive: IsAlive[A]) { self =>
  ...
  /**
   * Removes a value from long-term storage by clearing any WeakReference whose
   * referent equals `a`. Added in the legacy vendor to give the benchmark
   * harness a `remove` op symmetric with `FiberSet.remove`; the upstream
   * incumbent relies on GC to reclaim entries. Scan is linear in graduates size
   * — this accurately reflects the incumbent's lack of O(1) removal.
   */
  final def remove(a: A): Unit = {
    ...
    while (it.hasNext) {
      val ref   = it.next()
      val value = ref.get()
      if ((value eq null) || (value eq a)) it.remove()
    }
  }
