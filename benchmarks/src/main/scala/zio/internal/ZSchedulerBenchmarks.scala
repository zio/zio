...
class ZSchedulerBenchmarks {
  ...
  val catsRuntime     = IORuntime.global
  val fixedThreadPool = fixedThreadPoolExecutor(Platform.defaultYieldOpCount)
  val zScheduler      = Executor.makeDefault(Platform.defaultYieldOpCount)
