...
> Guide to ZIO's asynchronous interruption model: when fibers get interrupted, how interruptible and uninterruptible regions work, how finalizers and resource safety survive interruption, and how interruption is attributed in the resulting Cause.
...
| `ZIO#onInterrupt`               | `URIO[R, Any] => ZIO[R, E, A]`                                       | Runs a finalizer only when the cause is purely interruption (`Cause#isInterruptedOnly`).  | [Finalizers](interruption-and-finalizers.md#oninterrupt)                  |
