  ...
  def spec = suite("RaceOptimizationVerificationTest")(    
    test("optimized race should be significantly faster than cats-effect") {
      for {
        ...
        _ <- Console.printLine(s"Cats-Effect / Original ZIO = $catsToOriginalRatio")
        _ <- Console.printLine(s"Cats-Effect / Optimized ZIO = $catsToOptimizedRatio")
        _ <- Console.printLine(s"Original ZIO / Optimized ZIO = $originalToOptimizedRatio")
        ...
        _ <- if (catsToOptimizedRatio >= 5.0) {
          ...
          Console.printLine("\nVerification INCOMPLETE: The optimized race implementation does not yet achieve the 5x performance improvement goal.")
            .zipRight(Console.printLine(s"Current improvement: ${catsToOptimizedRatio}x (goal: 5x)"))
        }
      } yield {
