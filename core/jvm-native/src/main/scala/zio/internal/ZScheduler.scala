...
private final class ZScheduler(autoBlocking: Boolean) extends Executor { parent =>
  ...
  // Per-worker load counters with 16-long stride (128 bytes) to avoid false sharing.
  // Each worker's count lives on its own cache line, so the O(N) scan in
  // chooseWorker() reads N independent L3 lines without cross-worker invalidation.
  ...
  override def stealWork(depth: Int): Boolean = {
    ...
    if (worker ne null) {
      ...
      if (worker.nextRunnable ne null) {
        ...
        if (runnable ne null) {
          taskCounts.getAndDecrement(worker.workerIndex * 16)
        } else {
    ...
    while (i < poolSize) {
      ...
      if (!w.blocking) {
        ...
        if (load < minLoad) {
          minLoad = load
          best = w
          if (load == 0L) return best // short-circuit: empty worker found
        }
      }
      i += 1
    }
                  ...
                  if ((worker ne self) && !worker.blocking) {
                    ...
                    if (size > 0) {
                      ...
                      if (nRunnables > 0) {
                        ...
                        runnable = iter.next()
                        if (nRunnables > 1) {
                          val remainder = localQueue.offerAll(iter, nRunnables - 1)
                          val accepted  = (nRunnables - 1) - remainder.size
                          if (accepted > 0) taskCounts.getAndAdd(selfSlot, accepted.toLong)
                          if (!remainder.isEmpty) globalQueue.offerAll(remainder, random)
                        }
                        currentBlocking = blocking
                        if (currentBlocking) {
