...
private final class ZIOScheduler(autoBlocking: Boolean) extends Executor { parent =>
  ...
  private[this] val workers         = Array.ofDim[ZIOScheduler.Worker](poolSize)
  ...
  if (autoBlocking) {
    ...
    supervisor.setName("ZIOScheduler-Supervisor")
    supervisor.setDaemon(true)
    supervisor.start()
  }

  override private[zio] def isCurrentThreadInExecutor: Boolean =
    Thread.currentThread().isInstanceOf[ZIOScheduler.Worker]
  ...
  private def handleFullWorkerQueue(worker: ZIOScheduler.Worker, runnable: Runnable): Unit = {
  ...
  private[this] def isBlocking(worker: ZIOScheduler.Worker, runnable: Runnable): Boolean =
  ...
  private[this] def makeLocations(): ZIOScheduler.Locations =
    if (autoBlocking) new ZIOScheduler.Locations.Enabled
    else ZIOScheduler.Locations.Disabled

  private[this] def makeSupervisor(): ZIOScheduler.Supervisor =
    new ZIOScheduler.Supervisor {
  ...
  private[this] def makeWorker(): ZIOScheduler.Worker =
    new ZIOScheduler.Worker {
      ...
      final override def run(): Unit = {
        ...
        val poolSize    = ZIOScheduler.poolSize
      ...
      def registerChannel[T <: java.nio.channels.SelectableChannel](
        channel: T,
        ops: Int,
        attachment: AnyRef
      ): NioSelectionKey = {
...
private object ZIOScheduler {
  ...
  private def workerOrNull(): ZIOScheduler.Worker =
    Thread.currentThread() match {
      case w: ZIOScheduler.Worker => w
      case _                      => null
    }
  ...
  private sealed abstract class Worker extends Thread with BlockContext {
    ...
    final def setName(i: Int): Unit =
      setName(s"ZIOScheduler-Worker-$i")
