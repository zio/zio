...
class QueueSpec extends BaseCrossPlatformSpec {
  ...
  def e4 =
    for {
      ...
      l      <- queue.take.repeat(ZSchedule.recurs(9) *> ZSchedule.identity[Int].collectAll.immediately)
      _      <- f.join
  ...
  def e5 =
    (for {
      queue        <- Queue.bounded[Int](10)
      _            <- queue.offer(1).repeat(ZSchedule.recurs(9).immediately)
  ...
  def e6 =
    for {
      ...
      _      <- waitForSize(queue, 10)
      l <- queue.take
            .repeat(ZSchedule.recurs(9) *> ZSchedule.identity[Int].collectAll.immediately)
    } yield l must containTheSameElementsAs(values)
...
object QueueSpec {
  ...
  def waitForSize[A](queue: Queue[A], size: Int): ZIO[Clock, Nothing, Int] =
    (queue.size <* clock.sleep(10.millis)).repeat(ZSchedule.doWhile[Int](_ != size).immediately)
