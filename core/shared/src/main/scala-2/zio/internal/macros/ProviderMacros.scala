...
private[zio] class ProviderMacros(val c: blackbox.Context) extends ProviderMacroUtils {
  import c.universe._

  def injectImpl[F[_, _, _], R: c.WeakTypeTag, E, A](
    provider: c.Expr[ZProvider[_, E, _]]*
  ): c.Expr[F[Any, E, A]] =
    injectBaseImpl[F, Any, R, E, A](provider, "provide")

  def injectSomeImpl[F[_, _, _], R0: c.WeakTypeTag, R: c.WeakTypeTag, E, A](
    provider: c.Expr[ZProvider[_, E, _]]*
  ): c.Expr[F[R0, E, A]] = {
    assertEnvIsNotNothing[R0]()
    injectBaseImpl[F, R0, R, E, A](provider, "provide")
  }
...
private[zio] object MacroUnitTestUtils {
  def getRequirements[R]: List[String] = macro ProviderMacros.debugGetRequirements[R]

  def showTree(any: Any): String = macro ProviderMacros.debugShowTree
}
