...
private[zio] trait LayerMacroUtils {
  ...
  def constructLayer[R0: WeakTypeTag, R: WeakTypeTag, E](
    ...
    val builder = LayerBuilder[Type, LayerExpr](
      ...
      combineTypes = (_, _) => c.abort(NoPosition, "combineTypes invoked for Scala 2. This should not happen."),
      expandRIn = (_, _) => c.abort(NoPosition, "expandRIn invoked for Scala 2. This should not happen."),
