...
trait ZIOVersionSpecific[-R, +E, +A] { self: ZIO[R, E, A] =>
   ...
   * }}}
   */
  inline def injectCustom[E1 >: E](inline layer: ZLayer[_,E1,_]*): ZIO[ZEnv, E1, A] =
    ${LayerMacros.injectImpl[ZEnv, R, E1,A]('self, 'layer)}
   ...
   */
  inline def inject[E1 >: E](inline layer: ZLayer[_,E1,_]*): ZIO[Any, E1, A] =
    ${LayerMacros.injectImpl[Any,R,E1, A]('self, 'layer)}
...
private final class InjectSomePartiallyApplied[R0, -R, +E, +A](val self: ZIO[R, E, A]) extends AnyVal {
  ...
  ${LayerMacros.injectImpl[R0, R, E1, A]('self, 'layer)}
}
