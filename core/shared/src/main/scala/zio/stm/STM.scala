...
object STM {
  ...
  private[stm] def doneNow[E, A](exit: => ZSTM.internal.TExit[E, A]): STM[E, A] =
    ZSTM.doneNow(exit)

  private[stm] def failNow[E](e: E): STM[E, Nothing] =
    ZSTM.failNow(e)

  private[stm] def succeedNow[A](a: A): STM[Nothing, A] =
    ZSTM.succeedNow(a)
}
