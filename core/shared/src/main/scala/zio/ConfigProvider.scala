...
object ConfigProvider {
  ...
  object IndexedFlat {
    ...
    object ConfigPath {
      ...
      def patch(patch: Flat.PathPatch)(path: ConfigPath): Either[Config.Error, ConfigPath] =
    ...
    fromIndexedFlat(new IndexedFlat {
      ...
      def loadIndexed[A](path: IndexedFlat.ConfigPath, primitive: Config.Primitive[A], split: Boolean)(implicit
        ...
        for {
          ...
          results <- Flat.util.parsePrimitive(value, path.map(_.value), name.value, primitive, escapedSeqDelim, split)
        } yield results
      }
