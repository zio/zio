...
object Queue extends QueuePlatformSpecific {
  ...
  private def unsafeCreate[A](
    ...
    shutdownFlag: AtomicReference[Cause[Nothing]],
    strategy: Strategy[A]
  ...
  private final class QueueImpl[A](
    ...
    override def offer(a: A)(implicit trace: Trace): UIO[Boolean] =
      ZIO.suspendSucceed {
        val cause = shutdownFlag.get
        if (cause ne null) Exit.failCause(cause)
        else {
    ...
    override def shutdown(implicit trace: Trace): UIO[Unit] =
      ZIO.fiberIdWith(fiberId => shutdownCause(Cause.interrupt(fiberId)))
    ...
    override def shutdownCause(cause: Cause[Nothing])(implicit trace: Trace): UIO[Unit] =
      ZIO.fiberIdWith { fiberId =>
        if (shutdownFlag.compareAndSet(null, cause)) {
          ...
          strategy.shutdown(fiberId, cause)
        }
        Exit.unit
      }.uninterruptible

    override def isShutdown(implicit trace: Trace): UIO[Boolean] = ZIO.succeed(shutdownFlag.get ne null)
  ...
  private sealed abstract class Strategy[A] {
    ...
    def shutdown(fiberId: FiberId, cause: Cause[Nothing])(implicit trace: Trace, unsafe: Unsafe): Unit
