...
sealed abstract class Fiber[+E, +A] { self =>
   ...
   *   `IO[E, A]`
   */
  final def join(implicit trace: Trace): IO[E, A] = await.flatMap(IO.done(_)) <* inheritRefs
...
object Fiber extends FiberPlatformSpecific {
  ...
  def done[E, A](exit: => Exit[E, A]): Fiber.Synthetic[E, A] =
    new Fiber.Synthetic[E, A] {
      final def await(implicit trace: Trace): UIO[Exit[E, A]]                    = IO.succeedNow(exit)
      final def children(implicit trace: Trace): UIO[Chunk[Fiber.Runtime[_, _]]] = IO.succeedNow(Chunk.empty)
      final def id: FiberId                                                      = FiberId.None
      final def interruptAs(id: FiberId)(implicit trace: Trace): UIO[Exit[E, A]] = IO.succeedNow(exit)
      final def inheritRefs(implicit trace: Trace): UIO[Unit]                    = IO.unit
      final def poll(implicit trace: Trace): UIO[Option[Exit[E, A]]]             = IO.succeedNow(Some(exit))
    }
  ...
  def fromFuture[A](thunk: => Future[A])(implicit trace: Trace): Fiber.Synthetic[Throwable, A] =
    new Fiber.Synthetic[Throwable, A] {
      ...
      final def await(implicit trace: Trace): UIO[Exit[Throwable, A]] = Task.fromFuture(_ => ftr).exit
