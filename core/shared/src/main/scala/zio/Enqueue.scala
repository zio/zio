...
sealed trait Enqueue[-A] extends Serializable {
  ...
  /**
   * Shuts down the queue with the specified cause. Any fibers that are
   * suspended on `offer` or `take` will be failed with the specified cause.
   * Future calls to `offer*` and `take*` will be failed with the specified
   * cause.
   */
