...
sealed trait ZIO[-R, +E, +A] extends Serializable with ZIOPlatformSpecific[R, E, A] with ZIOVersionSpecific[R, E, A] {
  ...
  /**
   * Provides a provider to the ZIO effect, which translates it to another
   * level.
   */
  final def provide[E1 >: E, R0, R1](
    provider: => ZProvider[R0, E1, R1]
  )(implicit ev: R1 <:< R, trace: ZTraceElement): ZIO[R0, E1, A] =
    ZIO.suspendSucceed(provider.build.map(_.upcast(ev)).use(r => self.provideEnvironment(r)))
   ...
   *
   * val zio2 = zio.provideCustom(loggingProvider)
   * }}}
   */
   ...
   * val loggingProvider: ZProvider[Any, Nothing, Logging] = ???
   *
   * val zio2 = zio.provideCustomServices(loggingProvider)
   * }}}
   */
  @deprecated("use provideCustom", "2.0.0")
   ...
   *
   * val zio2 = zio.provideSome[Random](clockProvider)
   * }}}
   */
   ...
   * val clockProvider: ZProvider[Any, Nothing, Clock] = ???
   *
   * val zio2 = zio.provideSomeServices[Random](clockProvider)
   * }}}
   */
  @deprecated("use provideSome", "2.0.0")
  ...
  /**
   * Constructs a provider from this effect.
   */
  final def toProvider[A1 >: A](implicit
    ev1: Tag[A1],
    ev2: IsNotIntersection[A1],
    trace: ZTraceElement
  ): ZProvider[R, E, A1] =
    ZProvider.fromZIO[R, E, A1](self)

  /**
   * Constructs a provider from this effect, which must return one or more
   * services.
   */
  final def toProviderEnvironment[B](implicit
    ...
    ZProvider.fromZIOMany(self.map(ev))
...
object ZIO extends ZIOCompanionPlatformSpecific {
  ...
  def provide[RIn, E, ROut, RIn2, ROut2](builder: ZProvider[RIn, E, ROut])(
    ...
    zio.provideSomeServices[RIn with RIn2](ZProvider.environment[RIn2] ++ builder)
