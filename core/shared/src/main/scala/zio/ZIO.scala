...
sealed trait ZIO[-R, +E, +A]
  ...
  final def forever(implicit trace: Trace): ZIO[R, E, Nothing] =
    ZIO.whileLoop(true)(self *> ZIO.yieldNow)(_ => ()).asInstanceOf[ZIO[R, E, Nothing]]
  ...
  final def retryN(n: => Int)(implicit ev: CanFail[E], trace: Trace): ZIO[R, E, A] =
    ZIO.suspendSucceed {
      ...
      ZIO
        .whileLoop(result.isEmpty) {
          self.foldCauseZIO(
            cause =>
              cause.failureOrCause.fold(
                _ =>
                  if (i <= 0) Exit.failCause(cause)
                  else {
                    i -= 1
                    ZIO.yieldNow
                  },
  ...
  final def retryUntilZIO[R1 <: R](
    ...
    ZIO.suspendSucceed {
      ...
      ZIO
        .whileLoop(result.isEmpty)(
          self.foldZIO(
            e => f(e).flatMap(b => if (b) Exit.fail(e) else ZIO.yieldNow),
            { a =>
              result = Some(a)
              ZIO.unit
            }
