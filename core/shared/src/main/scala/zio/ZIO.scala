...
sealed trait ZIO[-R, +E, +A] extends Serializable with ZIOPlatformSpecific[R, E, A] with ZIOVersionSpecific[R, E, A] {
  ...
  final def provide[E1 >: E, R0, R1](
    ...
    ZIO.suspendSucceed(serviceBuilder.build.map(_.upcast(ev)).use(r => self.provideEnvironment(r)))
   ...
   */
  final def toServiceBuilder[A1 >: A](implicit ev: Tag[A1], trace: ZTraceElement): ZServiceBuilder[R, E, A1] =
   ...
   */
  @deprecated("use toServiceBuilder", "2.0.0")
  final def toLayer[A1 >: A](implicit ev: Tag[A1], trace: ZTraceElement): ZServiceBuilder[R, E, A1] =
    toServiceBuilder[A1]
