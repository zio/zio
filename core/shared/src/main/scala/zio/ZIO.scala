...
/**
 * A `ZIO[R, E, A]` value is an immutable value that lazily describes a workflow or job. The workflow requires some
 * environment `R`, and may fail with an error of type `E`, or succeed with a value of type `A`.
 *
 ...
 * }}}
 *
 * ZIO effects model resourceful interaction with the outside world, including synchronous, asynchronous, concurrent,
 * and parallel interaction.
 *
 * ZIO effects use a fiber-based concurrency model, with built-in support for scheduling, fine-grained interruption,
 * structured concurrency, and high scalability.
 *
 * To run an effect, you need a `Runtime`, which is capable of executing effects. Runtimes bundle a thread pool together
 * with the environment that effects need.
 */
sealed trait ZIO[-R, +E, +A] extends Serializable with ZIOPlatformSpecific[R, E, A] { self =>
  ...
  /**
   * Returns the logical conjunction of the `Boolean` value returned by this effect and the `Boolean` value returned by
   * the specified effect. This operator has "short circuiting" behavior so if the value returned by this effect is
   * false the specified effect will not be evaluated.
   */
  ...
  /**
   * Returns an effect that executes both this effect and the specified effect, in parallel, returning result of
   * provided effect. If either side fails, then the other side will be interrupted.
   */
  ...
  /**
   * Depending on provided environment, returns either this one or the other effect lifted in `Left` or `Right`,
   * respectively.
   */
  ...
  /**
   * Returns an effect that executes both this effect and the specified effect, in parallel, this effect result
   * returned. If either side fails, then the other side will be interrupted.
   */
  final def <&[R1 <: R, E1 >: E, B](that: ZIO[R1, E1, B]): ZIO[R1, E1, A] =
    self.zipWithPar(that)((a, _) => a)

  /**
   * Returns an effect that executes both this effect and the specified effect, in parallel, combining their results
   * into a tuple. If either side fails, then the other side will be interrupted.
   */
  final def <&>[R1 <: R, E1 >: E, B](that: ZIO[R1, E1, B]): ZIO[R1, E1, (A, B)] =
  ...
  /**
   * Returns the logical conjunction of the `Boolean` value returned by this effect and the `Boolean` value returned by
   * the specified effect. This operator has "short circuiting" behavior so if the value returned by this effect is true
   * the specified effect will not be evaluated.
   */
  final def ||[R1 <: R, E1 >: E](that: => ZIO[R1, E1, Boolean])(implicit ev: A <:< Boolean): ZIO[R1, E1, Boolean] =
  ...
  /**
   * Returns an effect that submerges the error case of an `Either` into the `ZIO`. The inverse operation of
   * `ZIO.either`.
   */
  ...
  /**
   * Returns a new effect that will not succeed with its value before first waiting for the end of all child fibers
   * forked by the effect.
   */
  final def awaitAllChildren: ZIO[R, E, A] = ensuringChildren(Fiber.awaitAll(_))

  /**
   * Returns an effect whose failure and success channels have been mapped by the specified pair of functions, `f` and
   * `g`.
   */
  ...
  /**
   * Uncurried version. Doesn't offer curried syntax and has worse type-inference characteristics, but it doesn't
   * allocate intermediate [[zio.ZIO.BracketAcquire_]] and [[zio.ZIO.BracketRelease_]] objects.
   */
  ...
  /**
   * Returns an effect that, if evaluated, will return the cached result of this effect. Cached results will expire
   * after `timeToLive` duration.
   */
  ...
  /**
   * Returns an effect that, if evaluated, will return the cached result of this effect. Cached results will expire
   * after `timeToLive` duration. In addition, returns an effect that can be used to invalidate the current cached value
   * before the `timeToLive` duration expires.
   */
   ...
   * }}}
   *
   * '''WARNING''': There is no sensible way to recover from defects. This method should be used only at the boundary
   * between ZIO and an external system, to transmit information on a defect for diagnostic or explanatory purposes.
   */
  ...
  /**
   * Returns an effect that succeeds with the cause of failure of this effect, or `Cause.empty` if the effect did
   * succeed.
   */
  ...
  /**
   * Taps the effect, printing the result of calling `.toString` on the value. Prefixes the output with the given
   * message.
   */
  ...
  /**
   * Returns an effect whose interruption will be disconnected from the fiber's own interruption, being performed in the
   * background without slowing down the fiber's interruption.
   *
   * This method is useful to create "fast interrupting" effects. For example, if you call this on a bracketed effect,
   * then even if the effect is "stuck" in acquire or release, its interruption will return immediately, while the
   * acquire / release are performed in the background.
   *
   ...
   *
   * The error parameter of the returned `ZIO` is `Nothing`, since it is guaranteed the `ZIO` effect does not model
   * failure.
   */
  ...
  /**
   * Returns an effect that, if this effect _starts_ execution, then the specified `finalizer` is guaranteed to begin
   * execution, whether this effect succeeds, fails, or is interrupted.
   *
   * For use cases that need access to the effect's result, see [[ZIO#onExit]].
   *
   * Finalizers offer very powerful guarantees, but they are low-level, and should generally not be used for releasing
   * resources. For higher-level logic built on `ensuring`, see `ZIO#bracket`.
   */
  ...
  /**
   * Acts on the children of this fiber (collected into a single fiber), guaranteeing the specified callback will be
   * invoked, whether or not this effect succeeds.
   */
  ...
  /**
   * Acts on the children of this fiber, guaranteeing the specified callback will be invoked, whether or not this effect
   * succeeds.
   */
  ...
  /**
   * Returns an effect that runs this effect and in case of failure, runs each of the specified effects in order until
  ...
  /**
   * Returns an effect that models the execution of this effect, followed by the passing of its value to the specified
   * continuation function `k`, followed by the effect that it returns.
   *
   * {{{
  ...
  /**
   * Creates a composite effect that represents this effect followed by another one that may depend on the error
  ...
  /**
   * Returns an effect that performs the outer effect first, followed by the inner effect, yielding the value of the
   * inner effect.
   *
  ...
  /**
   * Returns an effect that swaps the error/success cases. This allows you to use all methods on the error channel,
   * possibly before flipping back.
   */
  ...
  /**
   * Folds over the failure value or the success value to yield an effect that does not fail, but succeeds with the
   * value returned by the left or right function passed to `fold`.
   */
  ...
  /**
   * Recovers from errors by accepting one effect to execute for the case of an error, and one effect to execute for the
   * case of success.
   *
   * This method has better performance than `either` since no intermediate value is allocated and does not require
   * subsequent calls to `flatMap` to define the next effect.
   *
   * The error parameter of the returned `IO` may be chosen arbitrarily, since it will depend on the `IO`s returned by
   * the given continuations.
   */
  ...
  /**
   * Returns an effect that forks this effect into its own separate fiber, returning the fiber immediately, without
   * waiting for it to begin executing the effect.
   *
   * You can use the `fork` method whenever you want to execute an effect in a new fiber, concurrently and without
   * "blocking" the fiber executing other effects. Using fibers can be tricky, so instead of using this method directly,
   * consider other higher-level methods, such as `raceWith`, `zipPar`, and so forth.
   *
   * The fiber returned by this method has methods to interrupt the fiber and to wait for it to finish executing the
   * effect. See [[zio.Fiber]] for more information.
   *
   * Whenever you use this method to launch a new fiber, the new fiber is attached to the parent fiber's scope. This
   * means when the parent fiber terminates, the child fiber will be terminated as well, ensuring that no fibers leak.
   * This behavior is called "auto supervision", and if this behavior is not desired, you may use the [[forkDaemon]] or
   * [[forkIn]] methods.
   *
   * {{{
   * for {
  ...
  /**
   * Forks the effect into a new fiber attached to the global scope. Because the new fiber is attached to the global
   * scope, when the fiber executing the returned effect terminates, the forked fiber will continue running.
   */
  final def forkDaemon: URIO[R, Fiber.Runtime[E, A]] = forkIn(ZScope.global)

  /**
   * Forks an effect that will be executed without unhandled failures being reported. This is useful for implementing
   * combinators that handle failures themselves.
   */
  final def forkInternal: ZIO[R, Nothing, Fiber.Runtime[E, A]] =
    new ZIO.Fork(self, None, Some(_ => ()))

  /**
   * Forks the fiber in a [[ZManaged]]. Using the [[ZManaged]] value will execute the effect in the fiber, while
   * ensuring its interruption when the effect supplied to [[ZManaged#use]] completes.
   */
  ...
  /**
   * Returns a successful effect with the head of the list if the list is non-empty or fails with the error `None` if
   * the list is empty.
   */
  ...
  /**
   * Returns a new effect whose scope will be extended by the specified scope. This means any finalizers associated with
   * the effect will not be executed until the specified scope is closed.
   */
  ...
  /**
   * Returns a new effect that will not succeed with its value before first interrupting all child fibers forked by the
   * effect.
   */
  final def interruptAllChildren: ZIO[R, E, A] = ensuringChildren(Fiber.interruptAll(_))

  /**
   * Returns a new effect that performs the same operations as this effect, but interruptibly, even if composed inside
   * of an uninterruptible region.
   *
   * Note that effects are interruptible by default, so this function only has meaning if used within an uninterruptible
   * region.
   *
   * WARNING: This operator "punches holes" into effects, allowing them to be interrupted in unexpected places. Do not
   * use this operator unless you know exactly what you are doing. Instead, you should use [[ZIO.uninterruptibleMask]].
   */
  final def interruptible: ZIO[R, E, A] = interruptStatus(InterruptStatus.Interruptible)

  /**
   * Switches the interrupt status for this effect. If `true` is used, then the effect becomes interruptible (the
   * default), while if `false` is used, then the effect becomes uninterruptible. These changes are compositional, so
  ...
  /**
   * Returns an effect which is guaranteed to be executed on the specified executor. The specified effect will always
   * run on the specified executor, even in the presence of asynchronous boundaries.
   *
   * This is useful when an effect must be executed somewhere, for example: on a UI thread, inside a client library's
   * thread pool, inside a blocking thread pool, inside a low-latency thread pool, or elsewhere.
   *
   * The `lock` function composes with the innermost `lock` taking priority. Use of this method does not alter the
   * execution semantics of other effects composed with this one, making it easy to compositionally reason about where
   * effects are running.
   */
  ...
  /**
   * Returns an effect whose success is mapped by the specified side effecting `f` function, translating any thrown
  ...
  /**
   * Returns an effect with its error channel mapped using the specified function. This can be used to lift a "smaller"
   * error into a "larger" error.
   */
  final def mapError[E2](f: E => E2)(implicit ev: CanFail[E]): ZIO[R, E2, A] =
    self.foldCauseM(new ZIO.MapErrorFn(f), new ZIO.SucceedFn(f))

  /**
   * Returns an effect with its full cause of failure mapped using the specified function. This can be used to transform
   * errors while preserving the original structure of `Cause`.
   *
   * @see
  ...
  /**
   * Returns a new effect where the error channel has been merged into the success channel to their common combined
   * type.
   */
  ...
  /**
   * Runs the specified effect if this effect fails, providing the error to the effect if it exists. The provided effect
  ...
  /**
   * Propagates the success value to the first element of a tuple, but passes the effect input `R` along unmodified as
  ...
  /**
   * Returns this effect if environment is on the left, otherwise returns whatever is on the right unmodified. Note that
   * the result is lifted in either.
   */
  final def onLeft[C]: ZIO[Either[R, C], E, Either[A, C]] =
    self +++ ZIO.identity[C]

  /**
   * Returns this effect if environment is on the right, otherwise returns whatever is on the left unmodified. Note that
   * the result is lifted in either.
   */
  ...
  /**
   * Propagates the success value to the second element of a tuple, but passes the effect input `R` along unmodified as
  ...
  /**
   * Translates effect failure into death of the fiber, making all failures unchecked and not a part of the type of the
   * effect.
   */
  ...
  /**
   * Keeps none of the errors, and terminates the fiber with them, using the specified function to convert the `E` into
   * a `Throwable`.
   */
  ...
  /**
   * Returns an effect that will produce the value of this effect, unless it fails, in which case, it will produce the
   * value of the specified effect.
   */
  ...
  /**
   * Returns an effect that will produce the value of this effect, unless it fails with the `None` value, in which case
   * it will produce the value of the specified effect.
   */
  ...
  /**
   * Returns a new effect that will utilize the specified scope to supervise any fibers forked within the original
   * effect.
   */
  ...
  /**
   * Provides the part of the environment that is not part of the `ZEnv`, leaving an effect that only depends on the
   * `ZEnv`.
   *
   * {{{
  ...
  /**
   * Returns a new effect that will utilize the default scope (fiber scope) to supervise any fibers forked within the
   * original effect.
   */
  ...
  /**
   * Returns an effect that races this effect with the specified effect, returning the first successful `A` from the
   * faster side. If one effect succeeds, the other will be interrupted. If neither succeeds, then the effect will fail
   * with some error.
   *
   * WARNING: The raced effect will safely interrupt the "loser", but will not resume until the loser has been cleanly
   * terminated. If early return is desired, then instead of performing `l race r`, perform `l.disconnect race
   * r.disconnect`, which disconnects left and right interruption signals, allowing a fast return, with interruption
   * performed in the background.
   *
   * Note that if the `race` is embedded into an uninterruptible region, then because the loser cannot be interrupted,
   * it will be allowed to continue executing in the background, without delaying the return of the race.
   */
  ...
  /**
   * Returns an effect that races this effect with all the specified effects, yielding the value of the first effect to
   * succeed with a value. Losers of the race will be interrupted immediately
   */
  ...
  /**
   * Returns an effect that races this effect with the specified effect, yielding the first result to complete, whether
   * by success or failure. If neither effect completes, then the composed effect will not complete.
   *
   * WARNING: The raced effect will safely interrupt the "loser", but will not resume until the loser has been cleanly
   * terminated. If early return is desired, then instead of performing `l raceFirst r`, perform `l.disconnect raceFirst
   * r.disconnect`, which disconnects left and right interrupt signal, allowing a fast return, with interruption
   * performed in the background.
   */
  ...
  /**
   * Returns an effect that races this effect with the specified effect, yielding the first result to succeed. If
   * neither effect succeeds, then the composed effect will fail with some error.
   *
   * WARNING: The raced effect will safely interrupt the "loser", but will not resume until the loser has been cleanly
   * terminated. If early return is desired, then instead of performing `l raceEither r`, perform `l.disconnect
   * raceEither r.disconnect`, which disconnects left and right interrupt signal, allowing the earliest possible return.
   */
  final def raceEither[R1 <: R, E1 >: E, B](that: ZIO[R1, E1, B]): ZIO[R1, E1, Either[A, B]] =
    (self.map(Left(_)) race that.map(Right(_)))

  /**
   * Returns an effect that races this effect with the specified effect, calling the specified finisher as soon as one
   * result or the other has been computed.
   */
   ...
   *
   * Useful when joining fibers to make the resulting trace mention the `join` point, otherwise only the traces of
   * joined fibers are included.
   *
   * {{{
   *   for {
  ...
  /**
   * Continue with the returned computation if the `PartialFunction` matches, translating the successful match into a
   * failure, otherwise continue with our held value.
   */
   ...
   * Scheduled recurrences are in addition to the first execution, so that `io.repeat(Schedule.once)` yields an effect
   * that executes `io`, and then if that succeeds, executes `io` an additional time.
   */
  ...
  /**
   * Returns a new effect that repeats this effect according to the specified schedule or until the first failure, at
   * which point, the failure value and schedule output are passed to the specified handler.
   *
   * Scheduled recurrences are in addition to the first execution, so that `io.repeat(Schedule.once)` yields an effect
   * that executes `io`, and then if that succeeds, executes `io` an additional time.
   */
  ...
  /**
   * Retries with the specified retry policy. Retries are done following the failure of the original `io` (up to a fixed
   * maximum with `once` or `recurs` for example), so that that `io.retry(Schedule.once)` means "execute `io` and in
   * case of failure, try again once".
   */
  ...
  /**
   * Retries with the specified schedule, until it fails, and then both the value produced by the schedule together with
   * the last error are passed to the recovery function.
   */
  ...
  /**
   * Returns an effect that retries this effect with the specified schedule when it fails, until the schedule is done,
   * then both the value produced by the schedule together with the last error are passed to the specified recovery
   * function.
   */
  ...
  /**
   * Returns an effect that semantically runs the effect on a fiber, producing an [[zio.Exit]] for the completion value
   * of the fiber.
   */
   ...
   * }}}
   *
   * Using `sandboxWith` with `catchSome` is better than using `io.sandbox.catchAll` with a partial match, because in
   * the latter, if the match fails, the original defects will be lost and replaced by a `MatchError`
   */
  final def sandboxWith[R1 <: R, E2, B](f: ZIO[R1, Cause[E], A] => ZIO[R1, Cause[E2], B]): ZIO[R1, E2, B] =
    ZIO.unsandbox(f(self.sandbox))

  /**
   * Summarizes a effect by computing some value before and after execution, and then combining the values to produce a
   * summary, together with the result of execution.
   */
  ...
  /**
   * Returns an effect with the behavior of this one, but where all child fibers forked in the effect are reported to
  ...
  /**
   * Returns an effect that effectfully "peeks" at the success of this effect. If the partial function isn't defined at
   * the input, the result is equivalent to the original effect.
   *
   * {{{
  ...
  /**
   * Returns an effect that will timeout this effect, returning `None` if the timeout elapses before the effect has
   * produced a value; and returning `Some` of the produced value otherwise.
   *
   * If the timeout elapses without producing a value, the running effect will be safely interrupted.
   *
   * WARNING: The effect returned by this method will not itself return until the underlying effect is actually
   * interrupted. This leads to more predictable resource utilization. If early return is desired, then instead of using
   * `effect.timeout(d)`, use `effect.disconnect.timeout(d)`, which first disconnects the effect's interruption signal
   * before performing the timeout, resulting in earliest possible return, before an underlying effect has been
   * successfully interrupted.
   */
  final def timeout(d: Duration): ZIO[R with Clock, E, Option[A]] = timeoutTo(None)(Some(_))(d)

  /**
   * The same as [[timeout]], but instead of producing a `None` in the event of timeout, it will produce the specified
   * error.
   */
  ...
  /**
   * Returns an effect that will timeout this effect, returning either the default value if the timeout elapses before
   * the effect has produced a value; and or returning the result of applying the function `f` to the success value of
   * the effect.
   *
   * If the timeout elapses without producing a value, the running effect will be safely interrupted
   *
  ...
  /**
   * Returns an effect that keeps or breaks a promise based on the result of this effect. Synchronizes interruption, so
   * if this effect is interrupted, the specified promise will be interrupted, too.
   */
  ...
  /**
   * Enables ZIO tracing for this effect. Because this is the default, this operation only has an additional meaning if
   * the effect is located within an `untraced` section, or the current fiber has been spawned by a parent inside an
   * `untraced` section.
   */
  final def traced: ZIO[R, E, A] = tracingStatus(TracingStatus.Traced)

  /**
   * Toggles ZIO tracing support for this effect. If `true` is used, then the effect will accumulate traces, while if
   * `false` is used, then tracing is disabled. These changes are compositional, so they only affect regions of the
   * effect.
   */
  ...
  /**
   * When this effect succeeds with a cause, then this method returns a new effect that either fails with the cause that
   * this effect succeeded with, or succeeds with unit, depending on whether the cause is empty.
   *
  ...
  /**
   * Performs this effect uninterruptibly. This will prevent the effect from being terminated externally, but the effect
   * may fail for internal reasons (e.g. an uncaught error) or terminate due to defect.
   *
   * Uninterruptible effects may recover from all failure causes (including interruption of an inner effect that has
   * been made interruptible).
   */
  ...
  /**
   * Takes some fiber failures and converts them into errors, using the specified function to convert the `E` into an
   * `E1`.
   */
   ...
   *
   * Note: ZIO tracing is cached, as such after the first iteration it has a negligible effect on performance of
   * hot-spots (Additional hash map lookup per flatMap). As such, using `untraced` sections is not guaranteed to result
   * in a noticeable performance increase.
   */
  ...
  /**
   * Sequentially zips this effect with the specified effect using the specified combiner function. Combines the causes
   * in case both effect fail.
   */
  ...
  /**
   * Returns an effect that executes both this effect and the specified effect, in parallel, combining their results
   * with the specified `f` function. If both sides fail, then the cause will be combined.
   */
  ...
  /**
   * Returns an effect that executes both this effect and the specified effect, in parallel, combining their results
   * with the specified `f` function. If either side fails, then the other side will be interrupted.
   */
...
object ZIO extends ZIOCompanionPlatformSpecific {
   ...
   * `bracket` can be used to ensure the acquisition is not interrupted and the resource is always released.
   *
   * The function does two things:
   *
   *   1. Ensures this effect, which acquires the resource, will not be interrupted. Of course, acquisition may fail for
   *      internal reasons (an uncaught exception). 2. Ensures the `release` effect will not be interrupted, and will be
   ...
   *
   * If the `release` effect fails, then the entire effect will fail even if the `use` effect succeeds. If this
   * fail-fast behavior is not desired, errors produced by the `release` effect can be caught and ignored.
   *
   * {{{
  ...
  /**
   * Uncurried version. Doesn't offer curried syntax and have worse type-inference characteristics, but guarantees no
   * extra allocations of intermediate [[zio.ZIO.BracketAcquire]] and [[zio.ZIO.BracketRelease]] objects.
   */
  def bracket[R, E, A, B](
    acquire: ZIO[R, E, A],
  ...
  /**
   * Acquires a resource, uses the resource, and then releases the resource. Neither the acquisition nor the release
   * will be interrupted, and the resource is guaranteed to be released, so long as the `acquire` effect succeeds. If
   * `use` fails, then after release, the returned effect will fail with the same error.
   */
  def bracketExit[R, E, A](acquire: ZIO[R, E, A]): ZIO.BracketExitAcquire[R, E, A] =
    new ZIO.BracketExitAcquire(acquire)

  /**
   * Uncurried version. Doesn't offer curried syntax and has worse type-inference characteristics, but guarantees no
   * extra allocations of intermediate [[zio.ZIO.BracketExitAcquire]] and [[zio.ZIO.BracketExitRelease]] objects.
   */
  ...
  /**
   * Evaluate each effect in the structure from left to right, collecting the the successful values and discarding the
   * empty cases. For a parallel version, see `collectPar`.
   */
  ...
  /**
   * Evaluate each effect in the structure in parallel, collecting the the successful values and discarding the empty
   * cases.
   */
  ...
  /**
   * Similar to Either.cond, evaluate the predicate, return the given A as success if predicate returns true, and the
   * given E as error otherwise
   *
  ...
  /**
   * Returns an effect that dies with the specified `Throwable`. This method can be used for terminating a fiber because
   * a defect has been detected in the code.
   */
  def die(t: => Throwable): UIO[Nothing] =
    haltWith(trace => Cause.Traced(Cause.Die(t), trace()))

  /**
   * Returns an effect that dies with a [[java.lang.RuntimeException]] having the specified text message. This method
   * can be used for terminating a fiber because a defect has been detected in the code.
   */
  ...
  /**
   * Imports a synchronous side-effect into a pure `ZIO` value, translating any thrown exceptions into typed failed
   * effects creating with `ZIO.fail`.
   *
   * {{{
  ...
  /**
   * Imports an asynchronous side-effect into a pure `ZIO` value. See `effectAsyncMaybe` for the more expressive variant
   * of this function that can return a value synchronously.
   *
  ...
  /**
   * Imports an asynchronous side-effect into a ZIO effect. The side-effect has the option of returning the value
   * synchronously, which is useful in cases where it cannot be determined if the effect is synchronous or asynchronous
   * until the side-effect is actually executed. The effect also has the option of returning a canceler, which will be
   * used by the runtime to cancel the asynchronous effect if the fiber executing the effect is interrupted.
   *
   * If the register function returns a value synchronously, then the callback function `ZIO[R, E, A] => Any` must not
   * be called. Otherwise the callback function must be called at most once.
   *
   * The list of fibers, that may complete the async callback, is used to provide better diagnostics.
   */
  ...
  /**
   * Imports an asynchronous effect into a pure `ZIO` value. This formulation is necessary when the effect is itself
   * expressed in terms of `ZIO`.
   */
  ...
  /**
   * Returns a lazily constructed effect, whose construction may itself require effects. When no environment is required
   * (i.e., when R == Any) it is conceptually equivalent to `flatten(effect(io))`.
   */
  def effectSuspend[R, A](rio: => RIO[R, A]): RIO[R, A] = new ZIO.EffectSuspendPartialWith((_, _) => rio)

  /**
   * Returns a lazily constructed effect, whose construction may itself require effects. The effect must not throw any
   * exceptions. When no environment is required (i.e., when R == Any) it is conceptually equivalent to
   * `flatten(effectTotal(zio))`. If you wonder if the effect throws exceptions, do not use this method, use
   * [[Task.effectSuspend]] or [[ZIO.effectSuspend]].
   */
  ...
  /**
   * Imports a total synchronous effect into a pure `ZIO` value. The effect must not throw any exceptions. If you wonder
   * if the effect throws exceptions, then do not use this method, use [[Task.effect]], [[IO.effect]], or
   * [[ZIO.effect]].
   *
   * {{{
  ...
  /**
   * Filters the collection in parallel using the specified effectual predicate. See [[filter[R,E,A,Collection*]] for a
  ...
  /**
   * Filters the Set[A] in parallel using the specified effectual predicate. See [[filter[R,E,A,Collection*]] for a
   * sequential version of it.
   */
  ...
  /**
   * Filters the collection in parallel using the specified effectual predicate, removing all elements that satisfy the
   * predicate. See [[filterNot[R,E,A,Collection*]] for a sequential version of it.
   */
  ...
  /**
   * Filters the Set[A] in parallel using the specified effectual predicate, removing all elements that satisfy the
   * predicate. See [[filterNot[R,E,A](as:Set*]] for a sequential version of it.
   */
  ...
  /**
   * Returns an effect that runs the first effect and in case of failure, runs each of the specified effects in order
  ...
  /**
   * Returns an effect that first executes the outer effect, and then executes the inner effect, returning the value
   * from the inner effect, and effectively flattening a nested effect.
   */
   ...
   *
   * For a parallel version of this method, see `foreachPar`. If you do not need the results, see `foreach_` for a more
  ...
  /**
   * Applies the function `f` to each element of the `Map[Key, Value]` and returns the results in a new `Map[Key2,
   * Value2]`.
   *
   * For a parallel version of this method, see `foreachPar`. If you do not need the results, see `foreach_` for a more
   * efficient implementation.
  ...
  /**
   * Applies the function `f` to each element of the `NonEmptyChunk[A]` and returns the results in a new
   * `NonEmptyChunk[B]`.
   *
   * For a parallel version of this method, see `foreachPar`. If you do not need the results, see `foreach_` for a more
   * efficient implementation.
  ...
  /**
   * Applies the function `f` to each element of the `Collection[A]` and returns the result in a new `Collection[B]`
  ...
  /**
   * Applies the function `f` to each element of the `Collection[A]` in parallel, and returns the results in a new
   * `Collection[B]`.
   *
  ...
  /**
   * Applies the function `f` to each element of the `Array[A]` in parallel, and returns the results in a new
   * `Array[B]`.
   *
   * For a sequential version of this method, see `foreach`.
   */
  ...
  /**
   * Applies the function `f` to each element of the `Map[Key, Value]` in parallel and returns the results in a new
   * `Map[Key2, Value2]`.
   *
   * For a sequential version of this method, see `foreach`.
   */
  ...
  /**
   * Applies the function `f` to each element of the `NonEmptyChunk[A]` in parallel, and returns the results in a new
   * `NonEmptyChunk[B]`.
   *
   * For a sequential version of this method, see `foreach`.
   */
  ...
  /**
   * Applies the function `f` to each element of the `Iterable[A]` and runs produced effects in parallel, discarding the
   * results.
   *
   * For a sequential version of this method, see `foreach_`.
   *
   * Optimized to avoid keeping full tree of effects, so that method could be able to handle large input sequences.
  ...
  /**
   * Returns an effect that forks all of the specified values, and returns a composite fiber that produces a list of
  ...
  /**
   * Returns an effect that forks all of the specified values, and returns a composite fiber that produces unit. This
   * version is faster than [[forkAll]] in cases where the results of the forked fibers are not needed.
   */
  ...
  /**
   * Lifts an effectful function whose effect requires no environment into an effect that requires the input to the
   * function.
   */
  ...
  /**
   * Imports a [[scala.concurrent.Promise]] we generate a future from promise, and we pass to [fromFuture] to transform
   * into Task[A]
   */
  ...
  /**
   * Imports a function that creates a [[scala.concurrent.Future]] from an [[scala.concurrent.ExecutionContext]] into a
   * `ZIO`. The provided `ExecutionContext` will interrupt the `Future` between asynchronous operations such as `map`
   * and `flatMap` if this effect is interrupted. Note that no attempt will be made to interrupt a `Future` blocking on
   * a synchronous operation and that the `Future` must be created using the provided `ExecutionContext`.
   */
  ...
  /**
   * Lifts an `Option` into a `ZIO` but preserves the error as an option in the error channel, making it easier to
   * compose in some scenarios.
   */
  ...
  /**
   * Captures the fork scope, before overriding it with the specified new scope, passing a function that allows
   * restoring the fork scope to what it was originally.
   */
  ...
  /**
   * Makes the effect interruptible, but passes it a restore function that can be used to restore the inherited
   * interruptibility from whatever region the effect is composed into.
   */
  ...
  /**
   * Returns an effect that will execute the specified effect fully on the provided executor, before returning to the
   * default executor. See [[ZIO!.lock]].
   */
   ...
   *
   * It's unsafe to execute side effects inside `f`, as `f` may be executed more than once for some of `in` elements
  ...
  /**
   * Lifts an Option into a IO. If the option is empty it succeeds with Unit. If the option is defined it fails with the
   * content.
   */
  ...
  /**
   * Lifts an Option into a IO. If the option is empty it succeeds with Unit. If the option is defined it fails with an
   * error adapted with f.
   */
  def noneOrFailWith[E, O](o: Option[O])(f: O => E): IO[E, Unit] =
    getOrFailUnit(o).flip.mapError(f)

  /**
   * Feeds elements of type `A` to a function `f` that returns an effect. Collects all successes and failures in a
   * tupled fashion.
   */
  ...
  /**
   * Feeds elements of type `A` to a function `f` that returns an effect. Collects all successes and failures in
   * parallel and returns the result as a tuple.
   */
  ...
  /**
   * Given an environment `R`, returns a function that can supply the environment to programs that require it, removing
   * their need for any specific environment.
   *
  ...
  /**
   * Returns a effect that will never produce anything. The moral equivalent of `while(true) {}`, only without the
   * wasted CPU cycles. Fibers that suspended running this effect are automatically garbage collected on the JVM,
  ...
  /**
   * Replicates the given effect `n` times. If 0 or negative numbers are given, an empty `Iterable` will be returned.
   * This method is more efficient than using `List.fill` or similar methods, because the returned `Iterable` consumes
   * only a small amount of heap regardless of `n`.
   */
  ...
  /**
   * Requires that the given `ZIO[R, E, Option[A]]` contain a value. If there is no value, then the specified error will
   * be raised.
   */
  ...
  /**
   * Acquires a resource, uses the resource, and then releases the resource. However, unlike `bracket`, the separation
   * of these phases allows the acquisition to be interruptible.
   *
   * Useful for concurrent data structures and other cases where the 'deallocator' can tell if the allocation succeeded
   * or not just by inspecting internal / external state.
   */
  ...
  /**
   * Returns an effect that accesses the runtime, which can be used to (unsafely) execute tasks. This is useful for
   * integration with legacy code that must call back into ZIO code.
   */
  ...
  /**
   * Returns an effect that shifts execution to the specified executor. This is useful to specify a default executor
   * that effects sequenced after this one will be run on if they are not shifted somewhere else. It can also be used to
   * implement higher level operators to manage where an effect is run such as [[ZIO!.lock]] and [[ZIO!.on]].
   */
  def shift(executor: Executor): UIO[Unit] =
    new ZIO.Shift(executor)

  /**
   * Returns an effect that suspends for the specified duration. This method is asynchronous, and does not actually
   * block the fiber executing the effect.
   */
  ...
  /**
   * Transplants specified effects so that when those effects fork other effects, the forked effects will be governed by
   * the scope of the fiber that executes this effect.
   *
   * This can be used to "graft" deep grandchildren onto a higher-level scope, effectively extending their lifespans
  ...
  /**
   * Makes the effect uninterruptible, but passes it a restore function that can be used to restore the inherited
   * interruptibility from whatever region the effect is composed into.
   */
   ...
   *
   * Terminates with exceptions on the `Left` side of the `Either` error, if it exists. Otherwise extracts the contained
   * `IO[E, A]`
   */
   ...
   *
   * This combinator is lossy meaning that if there are errors all successes will be lost. To retain all information
   * please use [[partition]].
   */
  ...
  /**
   * Feeds elements of type `A` to `f `and accumulates, in parallel, all errors in error channel or successes in success
   * channel.
   *
   * This combinator is lossy meaning that if there are errors all successes will be lost. To retain all information
   * please use [[partitionPar]].
   */
  ...
  /**
   * Feeds elements of type `A` to `f`, in parallel, until it succeeds. Returns first success or the accumulation of all
   * errors.
   *
  ...
  /**
   * Locally installs a supervisor and an effect that succeeds with all the children that have been forked in the
   * returned effect.
   */
  ...
  /**
   * Returns an effect that yields to the runtime system, starting on a fresh stack. Manual use of this method can
   * improve fairness, at the cost of overhead.
   */
  ...
  implicit final class ZIOAutoCloseableOps[R, E, A <: AutoCloseable](private val io: ZIO[R, E, A]) extends AnyVal {
    ...
    /**
     * Like `bracket`, safely wraps a use and release of a resource. This resource will get automatically closed,
  ...
  implicit final class ZIOWithFilterOps[R, E, A](private val self: ZIO[R, E, A]) extends AnyVal {
    ...
    /**
     * Enables to check conditions in the value produced by ZIO If the condition is not satisfied, it fails with
     * NoSuchElementException this provide the syntax sugar in for-comprehension: for { (i, j) <- io1 positive <- io2 if
     * positive > 0 } yield ()
     */
  ...
  final class InterruptStatusRestore(private val flag: zio.InterruptStatus) extends AnyVal {
     ...
     * instantaneously. If the parent region is interruptible, then the effect can be interrupted normally, in the
     * foreground.
     */
