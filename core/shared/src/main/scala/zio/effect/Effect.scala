...
object Effect extends Serializable {
  ...
  trait Service extends Serializable {
     ...
     *
     * {{{
     * def putStrLn(line: String): Task[Unit] = Effect.Live.effect(println(line))
     * }}}
     */
     ...
     */
    def async[R, E, A](register: (ZIO[R, E, A] => Unit) => Unit): ZIO[R, E, A] =
     ...
     */
    def asyncMaybe[R, E, A](register: (ZIO[R, E, A] => Unit) => Option[ZIO[R, E, A]]): ZIO[R, E, A]
     ...
     * When no environment is required (i.e., when R == Any) it is conceptually equivalent to `flatten(effect(io))`.
     */
    def suspendWith[R, A](p: Platform => RIO[R, A]): RIO[R, A]
  ...
  trait Live extends Effect {
    ...
    val effect: Service = new Service {

      override def apply[A](effect: => A): Task[A] =
        new ZIO.EffectPartial(() => effect)

      override def asyncMaybe[R, E, A](
        register: (ZIO[R, E, A] => Unit) => Option[ZIO[R, E, A]]
      ): ZIO[R, E, A] =
        new ZIO.EffectAsync(register)

      override def suspend[R, A](rio: => RIO[R, A]): RIO[R, A] =
        new ZIO.EffectSuspendPartialWith(_ => rio)

      override def suspendTotal[R, E, A](zio: => ZIO[R, E, A]): ZIO[R, E, A] =
        new ZIO.EffectSuspendTotalWith(_ => zio)

      override def suspendTotalWith[R, E, A](p: Platform => ZIO[R, E, A]): ZIO[R, E, A] =
        new ZIO.EffectSuspendTotalWith(p)

      override def suspendWith[R, A](p: Platform => RIO[R, A]): RIO[R, A] =
