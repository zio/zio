 ...
 */
final class Semaphore0 private (private val state: Ref[State]) extends Serializable {
...
object Semaphore0 extends Serializable {

  /**
   * Creates a new `Sempahore` with the specified number of permits.
   */
  def make(permits: Long): UIO[Semaphore0] = UIO(unsafeMake(permits))

  private[zio] def unsafeMake(permits: Long): Semaphore0 = new Semaphore0(ZRef.unsafeMake[State](Right(permits)))
}
