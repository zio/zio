...
object Runtime {
  ...
  /**
   * Unsafely creates a `Runtime` from a `ZProvider` whose resources will be
   ...
   */
  def unsafeFromProvider[R](
    provider: Provider[Any, R],
    ...
    val (environment, shutdown) = runtime.unsafeRun {
      ZManaged.ReleaseMap.make.flatMap { releaseMap =>
        ZManaged.currentReleaseMap.locally(releaseMap)(provider.build.zio).flatMap { case (_, acquire) =>
          val finalizer = () =>
            runtime.unsafeRun {
   ...
   */
  @deprecated("Use unsafeFromProvider", "2.0.0")
  def unsafeFromLayer[R](
    layer: Layer[Any, R],
    runtimeConfig: RuntimeConfig = RuntimeConfig.default
  )(implicit trace: ZTraceElement): Runtime.Managed[R] =
    unsafeFromProvider(layer, runtimeConfig)
}
