...
/**
 * Stratified Epoch Collector (SEC) - High-performance concurrent weak set for
 * fibers.
 *
 * ==Design Insight==
 * Most fibers are "mayflies" (short-lived, <100ms). Only "vampires" (suspended
 * forever) need weak references for GC eligibility. SEC defers weak reference
 * allocation until epoch rotation, so mayflies NEVER allocate a WeakReference.
 *
 * ==Architecture==
 ...
 *
 * I2: Locator findability If fiber._setEpochId >= 0, epoch is discoverable and
 * slot references fiber
 *
 * I3: Publication order Store fiber in slot BEFORE updating fiber's locator
 * fields
 *
 * I4: Epoch discoverability epochMap.put(oldEpoch) BEFORE
 * activeEpoch.set(newEpoch)
 *
 * I5: Carry-forward before drop Never remove epoch from tracking without
 * rehoming live fibers first
 *
 * @param capacity
 *   slots per epoch (tune based on workload)
 */
final class FiberSet(capacity: Int = FiberSet.DefaultCapacity) {
   ...
   *
   * After this call returns, the fiber is visible to foreach() and can be
   * removed via remove().
   *
   ...
   *
   * Thread-safe. Handles concurrent rotation and carry-forward gracefully. If
   * removal cannot be confirmed after bounded retries, leaves locator intact
   * for eventual GC-based cleanup.
   *
  ...
  /**
   * Approximate size (for testing/debugging only). Not guaranteed to be
  ...
  /**
   * Attempt to rotate oldEpoch to archived status. Single-winner via CAS on
   * epoch state.
   *
   * Order is critical (I4):
   *   1. CAS state to ROTATING 2. Publish to epochMap (so remove() can find it)
   *      3. Swap activeEpoch 4. Convert survivors strong→weak 5. Set state to
   *      ARCHIVED 6. Push to archives 7. Enforce cap via carry-forward
   */
   ...
   *
   * This is the key correctness guarantee (I5): we NEVER drop an epoch while it
   * might still be the only membership record for live fibers.
   *
   ...
   *
   * Uses I3 ordering (store-then-locator). Does NOT invoke retirement logic to
   * avoid nested maintenance stacks.
   */
   ...
   *
   * Bounded to prevent iteration stalls. Tolerates retired epochs (simply
   * ignores refs from epochs no longer in epochMap).
   */
...
object FiberSet {
  ...
  /** Create a FiberSet with default capacity (4096 slots/epoch). */
