...
final class FiberRuntime[E, A](fiberId: FiberId.Runtime, fiberRefs0: FiberRefs, runtimeFlags0: RuntimeFlags)
   ...
   * '''NOTE''': This method must be invoked by the fiber itself.
   */
  private def drainQueueAfterAsync(runtimeFlags: RuntimeFlags)(implicit unsafe: Unsafe): ZIO[Any, Any, Any] = {
        ...
        try {
          cur match {
            ...
            case effect: Async[_, _, _] =>
              ...
              while (cur eq null) {
                cur = drainQueueAfterAsync(runtimeFlags)
