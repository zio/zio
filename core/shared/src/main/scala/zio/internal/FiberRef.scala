 ...
 *
 * In ZIO integration, this would be mixed into FiberRuntime.
 * The two locator fields enable O(1) removal.
 *
 * == Required Fields ==
 * Implementing classes must provide mutable storage for:
 * - `_setEpochId`: Long - which epoch this fiber is stored in (-1 if not in set)
...
trait FiberSetRef {
  ...
  /**
   * Epoch ID where this fiber is stored.
   * -1 means not currently in any FiberSet.
   */
   ...
   * Used during carry-forward to avoid rehoming dead fibers.
   */
  def isTerminated: Boolean
}

/**
 * Test implementation of FiberRef for unit testing.
