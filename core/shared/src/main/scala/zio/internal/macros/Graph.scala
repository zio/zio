...
import zio.internal.macros.DepsCompose._

final case class Graph[Key, A](nodes: List[Node[Key, A]], keyEquals: (Key, Key) => Boolean) {
  def buildComplete(outputs: List[Key]): Either[::[GraphError[Key, A]], DepsCompose[A]] =
  ...
  private def buildNode(node: Node[Key, A], seen: Set[Node[Key, A]]): Either[::[GraphError[Key, A]], DepsCompose[A]] =
    getDependencies(node).flatMap {
      forEach(_) { out =>
        assertNonCircularDependency(node, seen, out).flatMap(_ => buildNode(out, seen + out))
      }.map {
        _.distinct.combineHorizontally >>> DepsCompose.succeed(node.value)
      }
    }
