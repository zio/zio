 ...
 *   Add traces of ZIO.effect* operations in Full Execution Trace. Applies when
 *   `traceExecution` is enabled. May multiply the amount of memory used by the
 *   tracing cache.
 *
 * @param traceStack
 *   Collect trace of the current stack of future continuations, This trace
 *   resembles an imperative stacktrace and will usually include similar
 *   information, but due to the way ZIO tracing works, it includes only
 *   references to *future continuations*,
 ...
 *   How many lines of execution trace to include in the trace of last lines
 *   before .fork in the parent fiber that spawned the current fiber
 *
 * @param ancestorStackTraceLength
 *   How many lines of stack trace to include in the trace of last lines before
 *   .fork in the parent fiber that spawned the current fiber
 */
