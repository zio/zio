...
object MutableConcurrentQueue {
  ...
  /**
   * @note
   *   in case you need extreme performance, make sure to use capacity which is a power of 2 throughout your system.
   *   This will allow to use a more performant ring buffer implementation.
   */
 ...
 *   1. Should be non-blocking and ideally lock-free. 2. Should provide basic metrics such as how many elements were
 *      enqueued/dequeued.
 *
 * @note
...
protected[zio] abstract class MutableConcurrentQueue[A] {
   ...
   *
   * @note
   *   that if there's no meaningful default for your type, you can always use `poll(null)`. Not the best, but
   *   reasonable price to pay for lower heap churn from not using [[scala.Option]] here.
   */
