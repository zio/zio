...
trait TagLowPriority2 extends TagLowPriority3 {
  ...
  implicit def derive5[F[_, _, _, _, _], A, B, C, D, E](implicit tag: Tag[A], tag2: Tag[B], tag3: Tag[C], tag4: Tag[D], tag5: Tag[E], deriveTag: DeriveTag5[F]): Tag[F[A, B, C, D, E]] = {
...
trait TagLowPriority3 extends TagLowPriority4 {
  ...
  implicit def derive4[F[_, _, _, _], A, B, C, D](implicit tag: Tag[A], tag2: Tag[B], tag3: Tag[C], tag4: Tag[D], deriveTag: DeriveTag4[F]): Tag[F[A, B, C, D]] = {
    implicit val typeTag: izumi.reflect.Tag[A] = tag.typeTag
    Tag(???, ???)
  }
...
trait TagLowPriority4 extends TagLowPriority5 {
  implicit def derive3[F[_, _, _], A, B, C](implicit tag: Tag[A], tag2: Tag[B], tag3: Tag[C], deriveTag: DeriveTag3[F]): Tag[F[A, B, C]] = {
    implicit val typeTag: izumi.reflect.Tag[A] = tag.typeTag
    Tag(???, ???)
  }
}
...
final case class DeriveTag3[F[_, _, _]](typeTag: izumi.reflect.TagK3[F], isNotIntersection: IsNotIntersection[F[_, _, _]])
...
final case class DeriveTag4[F[_, _, _, _]](typeTag: izumi.reflect.TagK4[F], isNotIntersection: IsNotIntersection[F[_, _, _, _]])
...
final case class DeriveTag5[F[_, _, _, _, _]](typeTag: izumi.reflect.TagK5[F], isNotIntersection: IsNotIntersection[F[_, _, _, _, _]])
