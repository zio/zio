...
object TaskR {
   ...
   */
  final def checkInterruptible[R, A](f: Boolean => TaskR[R, A]): TaskR[R, A] =
   ...
   */
  final def flatten[R, A](zio: TaskR[R, TaskR[R, A]]): TaskR[R, A] =
    ZIO.flatten(zio)
   ...
   */
  final def interruptible[R, A](zio: TaskR[R, A]): TaskR[R, A] =
   ...
   */
  final def lock[R, A](executor: Executor)(zio: TaskR[R, A]): TaskR[R, A] =
   ...
   */
  final def raceAll[R, R1 <: R, A](zio: TaskR[R, A], ios: Iterable[TaskR[R1, A]]): TaskR[R1, A] =
    ZIO.raceAll(zio, ios)
   ...
   */
  final def supervise[R, A](zio: TaskR[R, A]): TaskR[R, A] =
    ZIO.supervise(zio)

  /** See [[scalaz.zio.ZIO.supervised]]
   */
  final def supervised[R, A](zio: TaskR[R, A]): TaskR[R, A] =
    ZIO.supervised(zio)
   ...
   */
  final def suspend[R, A](io: => TaskR[R, A]): TaskR[R, A] =
    ZIO.suspend(io)
   ...
   */
  final def uninterruptible[R, A](zio: TaskR[R, A]): TaskR[R, A] =
   ...
   */
  final def unsandbox[R, A](v: ZIO[R, Cause[Throwable], A]): TaskR[R, A] = ZIO.unsandbox(v)
