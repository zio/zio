...
object TaskR {
  ...
  final def superviseWith[R, A](
    zio: ZIO[R, TaskrE, A]
  )(supervisor: IndexedSeq[Fiber[_, _]] => ZIO[R, Nothing, _]): ZIO[R, TaskrE, A] =
  ...
  final def effectAsyncInterrupt[R, A](
    register: (ZIO[R, TaskrE, A] => Unit) => Either[Canceler, ZIO[R, TaskrE, A]]
  ): ZIO[R, TaskrE, A] =
  ...
  final def bracket[R, A, B](
    acquire: ZIO[R, TaskrE, A],
    release: A => ZIO[R, Nothing, _],
    use: A => ZIO[R, TaskrE, B]
  ): ZIO[R, TaskrE, B] = ZIO.bracket(acquire, release, use)
   ...
   */
  final def raceAll[R, R1 <: R, A](zio: ZIO[R, TaskrE, A], ios: Iterable[ZIO[R1, TaskrE, A]]): ZIO[R1, TaskrE, A] =
    ZIO.raceAll(zio, ios)

  /** Reduces an `Iterable[IO]` to a single `IO`, working sequentially.
   */
  final def reduceAll[R, R1 <: R, A](a: ZIO[R, TaskrE, A], as: Iterable[ZIO[R1, TaskrE, A]])(
    f: (A, A) => A
  ): ZIO[R1, TaskrE, A] =
   ...
   */
  final def reduceAllPar[R, R1 <: R, A](a: ZIO[R, TaskrE, A], as: Iterable[ZIO[R1, TaskrE, A]])(
    f: (A, A) => A
  ): ZIO[R1, TaskrE, A] =
   ...
   */
  def reserve[R, A, B](
    reservation: ZIO[R, TaskrE, Reservation[R, TaskrE, A]]
