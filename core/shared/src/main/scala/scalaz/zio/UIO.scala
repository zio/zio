...
object UIO {
   ...
   */
  final def superviseWith[A](
    zio: ZIO[UioR, UioE, A]
  )(supervisor: IndexedSeq[Fiber[_, _]] => ZIO[UioR, Nothing, _]): ZIO[UioR, UioE, A] =
  ...
  final def effectAsyncInterrupt[A](
    register: (ZIO[UioR, UioE, A] => Unit) => Either[Canceler, ZIO[UioR, UioE, A]]
  ): ZIO[UioR, UioE, A] =
   ...
   */
  final def bracket[A, B](
    acquire: ZIO[UioR, UioE, A],
    release: A => ZIO[UioR, Nothing, _],
    use: A => ZIO[UioR, UioE, B]
  ): ZIO[UioR, UioE, B] = ZIO.bracket(acquire, release, use)
  ...
  final def bracketExit[A, B](
    ...
    use: A => ZIO[UioR, UioE, B]
  ): ZIO[UioR, UioE, B] = ZIO.bracketExit(acquire, release, use)
   ...
   */
  final def raceAll[R1 <: UioR, A](zio: ZIO[UioR, UioE, A], ios: Iterable[ZIO[R1, UioE, A]]): ZIO[R1, UioE, A] =
    ZIO.raceAll(zio, ios)

  /** Reduces an `Iterable[IO]` to a single `IO`, working sequentially.
   */
  final def reduceAll[R1 <: UioR, A](a: ZIO[UioR, UioE, A], as: Iterable[ZIO[R1, UioE, A]])(
    f: (A, A) => A
  ): ZIO[R1, UioE, A] =
   ...
   */
  final def reduceAllPar[R1 <: UioR, A](a: ZIO[UioR, UioE, A], as: Iterable[ZIO[R1, UioE, A]])(
    f: (A, A) => A
  ): ZIO[R1, UioE, A] =
   ...
   */
  def reserve[A, B](
    reservation: ZIO[UioR, UioE, Reservation[UioR, UioE, A]]
  )(use: A => ZIO[UioR, UioE, B]): ZIO[UioR, UioE, B] =
