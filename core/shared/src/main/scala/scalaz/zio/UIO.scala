...
object UIO {
   ...
   */
  final def flatten[A](zio: UIO[UIO[A]]): UIO[A] =
    ZIO.flatten(zio)
   ...
   */
  final def interruptible[A](zio: UIO[A]): UIO[A] =
   ...
   */
  final def lock[A](executor: Executor)(zio: UIO[A]): UIO[A] =
   ...
   */
  final def raceAll[R1 <: Any, A](zio: UIO[A], ios: Iterable[ZIO[R1, Nothing, A]]): ZIO[R1, Nothing, A] =
    ZIO.raceAll(zio, ios)

  /** See [[scalaz.zio.ZIO.reduceAll]]
   */
  final def reduceAll[R1 <: Any, A](a: UIO[A], as: Iterable[ZIO[R1, Nothing, A]])(f: (A, A) => A): ZIO[R1, Nothing, A] =
    ZIO.reduceAll(a, as)(f)

  /** See [[scalaz.zio.ZIO.reduceAllPar]]
   */
  final def reduceAllPar[R1 <: Any, A](a: UIO[A], as: Iterable[ZIO[R1, Nothing, A]])(
    f: (A, A) => A
  ): ZIO[R1, Nothing, A] =
   ...
   */
  final def supervise[A](zio: UIO[A]): UIO[A] =
    ZIO.supervise(zio)

  /** See [[scalaz.zio.ZIO.supervised]]
   */
  final def supervised[A](zio: UIO[A]): UIO[A] =
    ZIO.supervised(zio)

  /** See [[scalaz.zio.ZIO.superviseWith]]
   */
  final def superviseWith[A](zio: UIO[A])(supervisor: IndexedSeq[Fiber[_, _]] => UIO[_]): UIO[A] =
    ZIO.superviseWith(zio)(supervisor)

  /** See [[scalaz.zio.ZIO.suspend]]
   */
  final def suspend[A](io: => UIO[A]): UIO[A] =
    ZIO.suspend(io)

  /** See [[scalaz.zio.ZIO.interruptibleMask]]
   */
  final def uninterruptible[A](zio: UIO[A]): UIO[A] =
   ...
   */
  final def when(b: Boolean)(zio: UIO[_]): UIO[Unit] =
    ZIO.when(b)(zio)

  /** See [[scalaz.zio.ZIO.whenM]]
   */
  final def whenM(b: UIO[Boolean])(zio: UIO[_]): UIO[Unit] =
    ZIO.whenM(b)(zio)
