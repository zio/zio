...
final class Ref[A] private (private val value: AtomicReference[A]) extends AnyVal with Serializable {
   ...
   */
  final def get: UIO[A] = BIO.effectTotal(value.get)

  /**
   * Writes a new value to the `Ref`, with a guarantee of immediate
   * consistency (at some cost to performance).
   */
  final def set(a: A): UIO[Unit] = BIO.effectTotal(value.set(a))

  /**
   * Writes a new value to the `Ref` without providing a guarantee of
   * immediate consistency.
   */
  final def setAsync(a: A): UIO[Unit] = BIO.effectTotal(value.lazySet(a))
   ...
   */
  final def update(f: A => A): UIO[A] = BIO.effectTotal {
    var loop    = true
   ...
   */
  final def updateSome(pf: PartialFunction[A, A]): UIO[A] = BIO.effectTotal {
    var loop    = true
    var next: A = null.asInstanceOf[A]
   ...
   * `update`.
   */
  final def modify[B](f: A => (B, A)): UIO[B] = BIO.effectTotal {
    var loop = true
   ...
   */
  final def modifySome[B](default: B)(pf: PartialFunction[A, (B, A)]): UIO[B] = BIO.effectTotal {
    var loop = true
    var b: B = null.asInstanceOf[B]
...
object Ref extends Serializable {
   ...
   */
  final def make[A](a: A): UIO[Ref[A]] = BIO.effectTotal(new Ref[A](new AtomicReference(a)))
}
