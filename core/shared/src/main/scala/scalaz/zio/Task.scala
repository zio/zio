...
object Task {
  ...
  final def superviseWith[A](
    zio: ZIO[TaskR, TaskE, A]
  )(supervisor: IndexedSeq[Fiber[_, _]] => ZIO[TaskR, Nothing, _]): ZIO[TaskR, TaskE, A] =
  ...
  final def effectAsyncMaybe[A](
    register: (ZIO[TaskR, TaskE, A] => Unit) => Option[IO[TaskE, A]]
  ): ZIO[TaskR, TaskE, A] =
  ...
  final def effectAsyncInterrupt[A](
    register: (ZIO[TaskR, TaskE, A] => Unit) => Either[Canceler, ZIO[TaskR, TaskE, A]]
  ): ZIO[TaskR, TaskE, A] =
  ...
  final def bracket[A, B](
    acquire: ZIO[TaskR, TaskE, A],
    release: A => ZIO[TaskR, Nothing, _],
    use: A => ZIO[TaskR, TaskE, B]
  ): ZIO[TaskR, TaskE, B] = ZIO.bracket(acquire, release, use)
   ...
   * R1? R?
   */
  final def raceAll[A](zio: ZIO[TaskR, TaskE, A], ios: Iterable[ZIO[TaskR, TaskE, A]]): ZIO[TaskR, TaskE, A] =
    ZIO.raceAll(zio, ios)

  /** Reduces an `Iterable[IO]` to a single `IO`, working sequentially.
   */
  final def reduceAll[A](a: ZIO[TaskR, TaskE, A], as: Iterable[ZIO[TaskR, TaskE, A]])(
    f: (A, A) => A
  ): ZIO[TaskR, TaskE, A] =
   ...
   */
  final def reduceAllPar[A](a: ZIO[TaskR, TaskE, A], as: Iterable[ZIO[TaskR, TaskE, A]])(
    f: (A, A) => A
  ): ZIO[TaskR, TaskE, A] =
   ...
   */
  def reserve[A, B](
    reservation: ZIO[TaskR, TaskE, Reservation[TaskR, TaskE, A]]
