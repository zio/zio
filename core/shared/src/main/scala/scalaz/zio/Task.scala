...
object Task {
   ...
   */
  final def done[A](r: Exit[Throwable, A]): IO[Throwable, A] = ZIO.done(r)
   ...
   */
  final def effectAsyncMaybe[A](register: (Task[A] => Unit) => Option[IO[Throwable, A]]): Task[A] =
   ...
   */
  final def fail(error: Throwable): IO[Throwable, Nothing] = ZIO.fail(error)

  /** See [[scalaz.zio.ZIO.flatten]]
   */
  final def flatten[A](zio: Task[Task[A]]): Task[A] =
    ZIO.flatten(zio)
   ...
   */
  final def fromFunctionM[A](f: Any => IO[Throwable, A]): Task[A] =
    ZIO.fromFunctionM(f)

  /** See [[scalaz.zio.ZIO.fromEither]]
   */
  final def fromEither[A](v: => Either[Throwable, A]): IO[Throwable, A] =
    ZIO.fromEither(v)

  /** See [[scalaz.zio.ZIO.fromFiber]]
   */
  final def fromFiber[A](fiber: => Fiber[Throwable, A]): IO[Throwable, A] =
    ZIO.fromFiber(fiber)

  /** See [[scalaz.zio.ZIO.fromFiberM]]
   */
  final def fromFiberM[A](fiber: IO[Throwable, Fiber[Throwable, A]]): IO[Throwable, A] =
   ...
   */
  final def halt(cause: Cause[Throwable]): IO[Throwable, Nothing] = ZIO.halt(cause)
   ...
   */
  final def interruptible[A](zio: Task[A]): Task[A] =
   ...
   */
  final def lock[A](executor: Executor)(zio: Task[A]): Task[A] =
   ...
   */
  final def raceAll[A](zio: Task[A], ios: Iterable[Task[A]]): Task[A] =
    ZIO.raceAll(zio, ios)
   ...
   */
  final def supervised[A](zio: Task[A]): Task[A] =
    ZIO.supervised(zio)

  /** See [[scalaz.zio.ZIO.supervise]]
   */
  final def supervise[A](zio: Task[A]): Task[A] =
    ZIO.supervise(zio)

  /** See [[scalaz.zio.ZIO.superviseWith]]
   */
  final def superviseWith[A](zio: Task[A])(supervisor: IndexedSeq[Fiber[_, _]] => UIO[_]): Task[A] =
   ...
   */
  final def uninterruptible[A](zio: Task[A]): Task[A] =
   ...
   */
  final def when(b: Boolean)(zio: Task[_]): Task[Unit] =
    ZIO.when(b)(zio)

  /** See [[scalaz.zio.ZIO.whenM]]
   */
  final def whenM(b: Task[Boolean])(zio: Task[_]): Task[Unit] =
    ZIO.whenM(b)(zio)
