...
trait CommonRTS {
   ...
   */
  def unsafeRunSync[E, A](io: IO[E, A]): ExitResult[E, A]

  def submit[A](block: => A): Unit

  def schedule[E, A](block: => A, duration: Duration): Async[E, Unit]
...
private object CommonRTS {
   ...
   */
  final class FiberContext[E, A](rts: CommonRTS, val unhandled: List[Throwable] => IO[Nothing, Unit])
      extends Fiber[E, A] {
    ...
    final def enterSupervision: IO[E, Unit] = IO.sync {
      ...
      def newWeakSet[A]: Set[A] = Collections.newSetFromMap[A](rts.getMap[A])
