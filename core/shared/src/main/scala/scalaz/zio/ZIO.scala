...
sealed trait ZIO[-R, +E, +A] extends Serializable { self =>
  ...
  final def repeatOrElseEither[R1 <: R, B, E2, C](
        ...
        a =>
          schedule.update(a, state).flatMap { step =>
            step.delay.run.flatMap { delay =>
              ...
              else clock.sleep(delay) *> loop(Some(() => step.finish()._2), step.state)
            }
          }
      )
    ...
    schedule.initial.flatMap {
      case (dl, state) =>
        dl.run.flatMap(dur => clock.sleep(dur) *> loop(None, state))
    }
  }
  ...
  final def retryOrElseEither[R1 <: R, E1 >: E, S, E2, B](
    ...
    policy.initial.flatMap {
      case (dl, state) =>
        dl.run.flatMap(dur => clock.sleep(dur) *> loop(state))
    }
  }
