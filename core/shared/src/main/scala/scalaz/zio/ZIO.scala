...
sealed abstract class ZIO[-R, +E, +A] extends Serializable { self =>
   ...
   */
  final def interruptible: ZIO[R, E, A] = new ZIO.Interruptible(self)
...
trait ZIOFunctions extends Serializable {
  ...
  final def effectAsyncInterrupt[R >: LowerR, E <: UpperE, A](
    ...
    effectTotal((new AtomicBoolean(false), OneShot.make[UIO[Any]])).flatMap {
      case (started, cancel) =>
        ...
        }.onInterrupt(flatten(effectTotal(if (started.get) cancel.get() else ZIO.unit))).uninterruptible
    }
  }
  ...
  final def bracket[R >: LowerR, R1 >: LowerR <: R, E <: UpperE, A, B](
    ...
    Ref.make[UIO[Any]](ZIO.unit).flatMap { m =>
      (for {
        r <- environment[R1]
        a <- acquire.interruptible.flatMap(a => m.set(release(a).provide(r)).const(a)).uninterruptible
        b <- use(a)
...
object ZIO extends ZIO_R_Any {
  ...
  final class Interruptible[R, E, A](val zio: ZIO[R, E, A]) extends ZIO[R, E, A] {
