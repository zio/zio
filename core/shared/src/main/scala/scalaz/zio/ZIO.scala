...
sealed trait ZIO[-R, +E, +A] extends Serializable { self =>
   ...
   */
  final def repeat[R1 <: R, B](schedule: ScheduleR[R1, A, B]): ZIO[R1 with Clock, E, B] =
   ...
   */
  final def retry[R1 <: R, E1 >: E, S](policy: ScheduleR[R1, E1, S]): ZIO[R1 with Clock, E1, A] =
