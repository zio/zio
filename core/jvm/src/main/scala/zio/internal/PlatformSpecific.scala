...
private[internal] trait PlatformSpecific {
   ...
   *
   * Tracing adds a constant ~2x overhead on FlatMaps, however, it's an optional feature and it's not valid to compare
   * the performance of ZIO with enabled Tracing with effect types _without_ a comparable feature.
   */
  lazy val benchmark: Platform = makeDefault(Int.MaxValue).withReportFailure(_ => ()).withTracing(Tracing.disabled)

  /**
   * The default platform, configured with settings designed to work well for mainstream usage. Advanced users should
   * consider making their own platform customized for specific application requirements.
   */
