...
trait JvmMetrics { self =>
  type Feature
  val featureTag: EnvironmentTag[Feature]
  ...
  lazy val live: ZLayer[Clock with System, Throwable, Feature] = {
    ...
    collectMetrics.toLayer(Tag(featureTag, IsNotIntersection[Feature]), IsNotIntersection[Feature], trace)
  }

  /** A ZIO application that periodically updates the JVM metrics */
  lazy val app: ZIOApp = new ZIOApp {
    @silent private implicit val ftag: EnvironmentTag[Feature] = featureTag
