...
class SinkSpec(implicit ee: org.specs2.concurrent.ExecutionEnv)
  ...
  private def foldShortCircuits = {
    val empty: Stream[Nothing, Int]     = StreamR.empty
    val single: Stream[Nothing, Int]    = StreamR.succeed(1)
    val double: Stream[Nothing, Int]    = StreamR(1, 2)
    val failed: Stream[String, Nothing] = StreamR.fail("Ouch")
