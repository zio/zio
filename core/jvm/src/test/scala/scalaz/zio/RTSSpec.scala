...
class RTSSpec(implicit ee: ExecutionEnv) extends TestRuntime {
  ...
  def testBracketRegression1 = {
    ...
    unsafeRun(for {
      ...
      f <- ZIO
            ...
            .fork
      _ <- (ref.get <* clock.sleep(1.millis)).repeat(ScheduleR.doUntil[List[String]](_.contains("start 1")))
      _ <- f.interrupt
      _ <- (ref.get <* clock.sleep(1.millis)).repeat(ScheduleR.doUntil[List[String]](_.contains("release 2")))
      l <- ref.get
  ...
  def testInterruptionRegression1 = {
    ...
    unsafeRun(
      for {
        f <- test.fork
        c <- (IO.effectTotal[Int](c.get) <* clock.sleep(1.millis)).repeat(ScheduleR.doUntil[Int](_ >= 1)) <* f.interrupt
      } yield c must be_>=(1)
    )
