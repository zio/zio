...
import org.openjdk.jcstress.infra.results.{II_Result, IIII_Result, IIIIII_Result}
...
object RingBufferConcurrencyTests {
  ...
  @Outcome.Outcomes(Array(
    ...
    new Outcome(id = Array("11, 21, 1, -1, 1, -1"), expect = ACCEPTABLE, desc = "First thread wins first offer, second thread wins second offer"),
    new Outcome(id = Array("21, 11, 1, -1, 1, -1"), expect = ACCEPTABLE, desc = "Second thread wins first offer, first thread wins second offer")
  ))
  @State
  ...
  @JCStressTest
  @Outcome.Outcomes(Array(
    new Outcome(id = Array("-10, -11, -20, -21"), expect = ACCEPTABLE, desc = "Both pollers finish before offer starts"),
