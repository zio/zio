...
object AccessibleMacroExample {
  ...
  trait Service {
    ...
    def sink(n: Int): ZSink[Any, Nothing, Nothing, Int, List[Int]]
  }
  ...
  val live: ZLayer[Console, Nothing, Has[Service]] =
    ZLayer.fromService(console => new Service {
      ...
      def sink(n: Int): ZSink[Any, Nothing, Nothing, Int, List[Int]] = ZSink.collectAll
    })

  // can use accessors even in the same compilation unit
  val program: URIO[AccessibleMacroExample with Random, (Int, String, Long, List[Foo], Int, String, String, ZStream[Any, String, Int], ZSink[Any, Nothing, Nothing, Int, List[Int]])] =
    for {
