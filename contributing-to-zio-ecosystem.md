# Contributing to the ZIO Ecosystem Projects
...
This CI job uses **npm Trusted Publishing with OpenID Connect (OIDC)** for secure authentication. No npm token secret is required. Instead:

1. The workflow requests an OIDC token from GitHub (via the `id-token: write` permission)
2. The `setup-node` action configures npm to use this OIDC token for authentication
3. The npm package on npmjs.com must be configured to accept OIDC tokens from your repository

**Setup Instructions for npm Trusted Publisher:**

1. Go to your npm package settings page: `https://www.npmjs.com/package/@zio.dev/<package-name>/settings/access`
2. Add a **Trusted Publisher** with:
   - **Provider**: GitHub Actions
   - **Organization**: `zio`
   - **Repository**: `<your-repo-name>`
   - **Workflow filename**: `ci.yml` (or your actual workflow filename - must be exact, not a wildcard)
   - **Environment name**: (leave blank unless you use a specific environment)

For more information, see the [npm Trusted Publishers documentation](https://docs.npmjs.com/trusted-publishers/).
...
The ZIO SBT CI plugin can generate the CI jobs automatically, including the job for publishing the documentation package to the NPM registry using npm Trusted Publishing with OIDC.
...
**Important:** After generating the workflow, you must configure your npm package as a **Trusted Publisher** on npmjs.com:

1. Go to: `https://www.npmjs.com/package/@zio.dev/<package-name>/settings/access`
2. Add a **Trusted Publisher** with:
   - **Provider**: GitHub Actions
   - **Organization**: `zio`
   - **Repository**: `<your-repo-name>`
   - **Workflow filename**: `ci.yml` (exact match required, not a wildcard)
...
The generated CI uses npm Trusted Publishing with OIDC for secure, token-free authentication. No NPM_TOKEN secret is needed.
:::
