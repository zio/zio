...
object DeriveGen {
    def apply[A](using DeriveGen[A]): Gen[Has[Random] with Has[Sized], A] =
        summon[DeriveGen[A]].derive

    inline def instance[A](gen: => Gen[Has[Random] with Has[Sized], A]): DeriveGen[A] =
        new DeriveGen[A] {
    ...
    inline def gen[T](using m: Mirror.Of[T]): DeriveGen[T] =
        new DeriveGen[T] {
            def derive: Gen[Has[Random] with Has[Sized], T] = {
    ...
    def genSum[T](s: Mirror.SumOf[T], instances: => List[DeriveGen[_]]): Gen[Has[Random] with Has[Sized], T] =
        Gen.suspend(Gen.oneOf(instances.map(_.asInstanceOf[DeriveGen[T]].derive) : _*))

    def genProduct[T](p: Mirror.ProductOf[T], instances: => List[DeriveGen[_]]): Gen[Has[Random] with Has[Sized], T] =
        Gen.suspend(
