...
/**
 * Basic benchmark to verify if an optimized race implementation in ZIO can achieve the 5x performance goal
 ...
 */
object BasicRaceBenchmark extends ZIOAppDefault {
  ...
  /**
   * A simple optimized race implementation that uses a more efficient approach
   * by only forking one fiber instead of two.
   */
  def optimizedRace[R, E, A](left: ZIO[R, E, A], right: ZIO[R, E, A]): ZIO[R, E, A] = {
    for {
      promise <- Promise.make[E, A]
      rightFiber <- right.fork
      _ <- left.foldZIO(
        e => promise.fail(e) *> ZIO.fail(e),
        a => promise.succeed(a) *> ZIO.succeed(a)
      ).ensuring(rightFiber.interrupt).fork
      result <- promise.await.onInterrupt(rightFiber.interrupt)
    } yield result
  }
  
  /**
   * Benchmark for our optimized race implementation
   */
  def runOptimizedRace(): Long = {
    val startTime = java.lang.System.nanoTime()
    
    def loop(i: Int): UIO[Int] =
      if (i < iterations) optimizedRace(ZIO.never, ZIO.succeed(i + 1)).flatMap(_ => loop(i + 1))
      else ZIO.succeed(i)
