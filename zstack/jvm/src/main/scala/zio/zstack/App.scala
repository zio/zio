...
object App {
  final def shutdown(selector: Selector, socket: ServerSocketChannel) = {
    socket.close()
    selector.close()
    println("zstack shutdown")
  }
  final def main(args: Array[String]): Unit = {
    val config = args.sliding(2, 1).toList.foldLeft(ZStackConfig.empty) { case (accumArgs, currArgs) => currArgs match {
    ...
    println(s"Starting zstack server on host: [${config.host}] port: [${config.port}]")
    ZStackServer(config, selector, socket, address)
 }
}
